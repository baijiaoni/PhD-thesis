...
The average radial excursion is calculated for the four cases of rf frequency modulations by eq.~(\ref{eq:phaseR}). Fig.~\ref{radial} shows the calculation result. 
\begin{figure}[H]
   \centering   
...
\begin{figure}[H]
   ...
   \label{bucket_size}
\end{figure}
Tab. \ref{bucket size} shows the minimum bucket area factor for four cases. For all cases, the running bucket area factor is larger than 85$\%$. Hence, all cases are applicable. 
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
...
A Virtual Local Area Network (\gls{VLAN}) is a group of FECs in the \gls{WR} network that is logically segmented by function or application, without regard to the physical locations of the FECs. For the WR network for FAIR, three VLANs with different priorities are applied according to the importance of the traffic. 
...
\begin{itemize}
	\item Trigger time $<$ the firing time of the extraction kicker of the source synchrotron

	\item Trigger time $<$ the firing time of the injection kicker of the target synchrotron

	\item Firing time of the extraction kicker $<$ the firing time of the injection kicker
\end{itemize}
