...
The dispersion function, a lattice parameter, defines the local sensitivity of the beam trajectory to a relative energy error ~\cite{lee_accelerator_2011}. For the rf frequency modulation of the phase shift method, the dispersion function is reflected in the relative momentum shift. The maximum tolerable relative momentum shift is decided by the semi-aperture required for the beam and the dispersion function. The maximum tolerable relative momentum of the $H^{+}$ beam and that of the $U^\mathit{28+}$ beam of the SIS18 are the same by coincident, $\Delta p/p_\mathit{\_max}=\pm0.008$, which is given by machine physicists. For the frequency beating method ($\Delta p/p=0$), the dispersion function is reflected in the relative bending magnetic field shift instead of the relative momentum shift. The maximum tolerable relative bending magnetic field shift is decided by the semi-aperture required for the beam and the dispersion function. The maximum tolerable relative bending magnetic field shift of the $H^{+}$ beam and that of the $U^\mathit{28+}$ beam of the SIS18 are minus of their maximum tolerant relative momentum shift, namely $\Delta B/B_\mathit{\_max}=-\Delta p/p_\mathit{\_max}=\pm0.008$. The constraint on the displacement of the orbit length $\Delta L/L_\mathit{\_max}$ is obtained by the product $\alpha_p \Delta p/p_\mathit{\_max}$ or $-\alpha_p \Delta B/B_\mathit{\_max}$. The reasonable bucket size of a running bucket is larger than $80\%$ of that of a stationary bucket, namely $\alpha_b(\phi_{s})\ge 80\%$. Due to the constraint of the bucket size, the synchronous phase must be within the range between $-6.4^\circ$ and $+6.4^\circ$.
...
The orbit length displacement is calculated for the three cases by eq.~(\ref{eq:phaseR}). Fig.~\ref{radial} shows the calculation result ~\cite{bai12_first_2014}. 
\begin{figure}[H]
   \centering   
...
For the frequency beating method, the rf frequency detuning is done during the SIS18 rf ramp or at the rf flattop. Due to the SIS18 $U^\mathit{28+}$ lattice and the semi-aperture required for the beam, the SIS18 $U^\mathit{28+}$ accepted bending magnetic field shift is~\cite{liebermann_fair_2013}
\begin{equation}
...
\begin{center}
    ...
    \hline
     \tabincell{c}{Maximum \\orbit length\\ displacement} & \tabincell{c}{Relative\\moment\\ shift} &\tabincell{c}{Synchronous\\phase} & \tabincell{c}{Bucket\\size} &\tabincell{c}{Adiabaticity}  \\ \hline
...
\end{equation}
The chromatic tune shift to the tune for three cases are negligible small.
...
Due to the SIS18 $H^+$ lattice and the semi-aperture required for the beam,, the SIS18 $H^+$ accepted bending magnetic field shift is
\begin{equation}
\frac{\Delta{B}}{B}=-\frac{1}{\alpha_p}\frac{\Delta{L}}{L} = \pm  8 \cdot 10^{-3}
%\gls{symb:radius} = \pm 2.4 \cdot 10^{-4}
\end{equation}
...
The longer the time is used for the phase extrapolation, the smaller the uncertainty of the extrapolated phase will be. When the PAP module uses \SI{500}{\micro\second} for the phase extrapolation, the uncertainty of the extrapolated phase is \SI{100}{ps} in the time domain ~\cite{ferrand_development_????}. The uncertainty of the extrapolated phase from the time domain to the phase domain (denoted as \gls{symb:un_h1phase100}) is calculated by eq. ~\ref{jitter_measure_p}.
...
\subsubsection{Requirement of B2B Traffic}
...
\begin{center}
    \begin{tabular}{ | c | c | c | }
    \hline
     \tabincell{c}{} & \tabincell{c}{Frame error rate} & \tabincell{c}{Upper bound latency of WR network}  \\ \hline
       \tabincell{c}{B2B \\ Broadcast} & $<1.6\cdot10^{-11}$ & \SI{500}{\us}  \\ \hline
		\tabincell{c}{B2B \\ Unicast} & $<1.4\cdot10^{-9}$ & \SI{500}{\us} \\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\subsubsection{Test Result of Frame Error Rate }
\begin{itemize}
    \item Frame error rate of the B2B Unicast frames

The FER of the B2B Unicast traffic is measured from the port 17 to the port 1. The B2B Unicast traffic has no loss frame, no misordered frame and no payload error frame, so the FER of the B2B Unicast traffic is $0$. 

    \item Frame error rate of the B2B Broadcast frames

The FER of the B2B Broadcast traffic is measured from the port 16 to other ports. For the B2B Broadcast frames, there is no misordered frame and no payload error frame. Hence, the FER is caused only by the loss frame. There is frame loss from the port 16 to the port 11, 12, 17 and 18. The frame loss rate is defined as the percentage of the number of lost frames to the total number of B2B Broadcast transferred frames on a specified port over the test duration. The frame loss rate from the port 16 to the port 11, 12, 17 and 18 is $1.78\cdot 10^{-8}$, see Fig. ~\ref{packet_loss}. 
...
According to the frame loss rate, the FER can be calculated
\begin{equation}
	FER_\mathit{Broadcast}=\frac{1.78\cdot 10^{-8}\cdot4}{17}=4.2\cdot10^{-9}
\end{equation}

\end{itemize}
Compared with the FER requirement in Tab. ~\ref{requirement_network}, the B2B Unicast traffic meets the requirement and the B2B Broadcast traffic does not meet the requirement. 

\subsubsection{Test Result of Frame Transfer Latency}
...
Fig. ~\ref{Max_latency_jitter} and Tab. \ref{max latency jitter} shows the test result for the maximum frame transfer latency of the B2B Broadcast frames. 
...
From the test result, we get the conclusion that every WR switch causes a maximum \SI{30}{\us} delay.
...
For the B2B Unicast frames, the 4 WR switch network has approximate \SI{23}{\us} maximum frame transfer latency.
\end{itemize}
...
Tab. ~\ref{result} summarizes the test result. 
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | }
    \hline
     \tabincell{c}{} & \tabincell{c}{Frame error rate} & \tabincell{c}{Maximum frame transfer latency} \\ \hline
       \tabincell{c}{B2B Broadcast} &  $4.2\cdot10^{-9}$ & \SI{30}{\us}/switch \\ \hline
		\tabincell{c}{B2B Unicast} 	 & $0$ & \tabincell{c}{\SI{23}{\us}/4switch}\\ \hline
    \end{tabular}
\end{center}
\end{table}

The frame loss rate of the B2B Unicast frames meets the requirement. But the frame loss rate of the B2B Broadcast frames doesn't meet the requirement. The firmware of the WR switch is still under development by CERN.The frame loss rate for the B2B Broadcast frames doesn't meet the requirement. The firmware of the WR switch is still under development by CERN. 


For the B2B transfer system, the upper bound latency of the frames on the WR network is \SI{500}{\us}, see Tab.\ref{requirement_network}. According to the test result, the maximum frame transfer latency for each WR layer is \SI{30}{\micro\second}. The latency of the WR network is decided by the layers of WR switches and the length of the optical fiber. The propagation of light through the core of an optical fiber is roughly about \SI{200}{\meter/\us}~\cite{_calculating_2012} and the longest distance in the FAIR campus is around \SI{2}{\kilo\meter}, so the latency of a \SI{2}{\kilo\meter} optical fiber is about \SI{10}{\us}. The layers of WR switches play a more important role in the latency. 

Here we calculate the tolerable layer of the WR switch between the B2B related \gls{SCU}s and between the B2B related SCUs and DM.  
		\begin{equation}
		\begin{aligned}
			\frac{\SI{500}{\us}-\SI{10}{\us}}{\SI{30}{\us/switch}}> 16
		\label {num_switch_b}
		\end{aligned}
		\end{equation}
