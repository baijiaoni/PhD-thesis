...
%\begin{eqnarray}\label{case4}
%\begin{aligned}
%\delta \Delta \phi_\mathit{syn}=\sqrt {(\frac {\partial \Delta \phi_\mathit{syn}}{\partial \psi^\mathit{trg}}\delta \psi^\mathit{trg})^2 + (\frac {\partial \Delta \phi_\mathit{syn}}{\partial \psi^\mathit{src}}\delta \psi^\mathit{src})^2} =\\
%\begin{cases}
%\sqrt {(\delta \psi^\mathit{trg})^2 + (\delta \psi^\mathit{src})^2}, &f_{\mathit{B2B}}^{trg}=f_{\mathit{syn}}^{trg}\cr
%\frac{f_{\mathit{syn}}^{trg}}{f_{\mathit{rev}}^{trg}}\sqrt {(\delta \psi^\mathit{trg})^2 + (\delta \psi^\mathit{src})^2}\mod 2\pi, &f_{\mathit{B2B}}^{trg}=f_{\mathit{rev}}^{trg}\cr
%\end{cases}
%\end{aligned}
%\end{eqnarray}

Both the B2B source SCU and the B2B target SCU measures the timestamp $t_\psi^X$ for the extrapolated phase and the uncertainty of the measured timestamp is \SI{1}{ns} (denoted as \gls{symb:uncertainty_time}).  
\begin{equation} 
\delta t_\psi^X= 1ns
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c |}
	    \hline
	\tabincell{c}{ Use cases of FAIR accelerators}& $\delta \psi^{X}$  & $|f_{\mathit{syn}}^\mathit{src}-f_{\mathit{syn}}^\mathit{trg}|$ & $\delta t_\mathit{align}$ & $T_\mathit{w}$\\ \hline
   \tabincell{c}{$U^{28+}$ B2B transfer\\ from the SIS18 to the SIS100}& $0.006^\circ$  &\SI{200}{Hz} & \SI{1.174}{us} & \SI{6.359}{us}\\ \hline
	\tabincell{c}{$H^{+}$ B2B transfer\\ from the SIS18 to the SIS100}& $0.010^\circ$  & \SI{200}{Hz} & \SI{0.983}{us}& \SI{3.678}{us} \\ \hline
	\tabincell{c}{h=4 B2B transfer\\ from the SIS18 to the ESR}& $0.025^\circ$ &  \SI{1899}{Hz} & \SI{0.103}{us}& \SI{1.456}{us} \\ \hline
	\tabincell{c}{h=1 B2B transfer\\ from the SIS18 to the ESR}& $0.036^\circ$ & \SI{1368}{Hz} & \SI{0.104}{us}& \SI{1.017}{us} \\ \hline	
	\tabincell{c}{B2B transfer \\from the ESR to the CRYRING}&$0.025^\circ$  & \SI{949}{Hz} & \SI{0.102}{us}& \SI{1.456}{us} \\ \hline	
	\tabincell{c}{$H^{+}$ B2B transfer \\from the SIS100 to the CR\\ via a Pbar} &$0.002^\circ$ & \SI{450}{Hz} & \SI{0.019}{us}& \SI{18.226}{us} \\ \hline	
	\tabincell{c}{RIB B2B transfer\\from the SIS100 to the CR \\ via a Super FRS}&$0.004^\circ$ &  \SI{108}{Hz} & \SI{0.154}{us} & \SI{9.779}{us}\\ \hline	
	\tabincell{c}{Antiproton B2B transfer \\from the CR to the HESR }&$0.004^\circ$ & \SI{136}{Hz} & \SI{0.123}{us}& \SI{9.860}{us} \\ \hline	
	\tabincell{c}{B2B transfer \\from the SIS18 to the ESR \\via a FRS}&$0.008^\circ$ & \SI{4249}{Hz} & \SI{7}{ns}& \SI{4.553}{us} \\ \hline	
   \end{tabular}
\end{center}
\end{table} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Uncertainty of Start of the Synchronization Window}
The start of the synchronization window with the phase shift method and the frequency beating method are calculated by eq. ~\ref{syn_win_start2} and  eq. ~\ref{syn_win_start3}. More explaination, please see Chap.  ~\ref{concept}.
\begin{equation}
t_\mathit{w\_start}=t_\psi^\mathit{X}+T-\frac{T_w}{2}-t_\mathit{delay}=t_\mathit{align}-\frac{T_w}{2}-t_\mathit{delay}\label{syn_win_start2}
\end{equation}
\begin{equation}
t_\mathit{w\_start}= t_\psi^\mathit{X}+\frac{\Delta \phi_\mathit{shift}}{2\pi}\cdot\frac{1}{|f_{\mathit{syn}}^\mathit{src}-f_{\mathit{syn}}^\mathit{trg}|}-\frac{T_w}{2}-t_\mathit{delay}=t_\mathit{align}-\frac{T_w}{2}-t_\mathit{delay}\label{syn_win_start3}
\end{equation}

The synchronization window is used to select the first rising edge of the bucket indication signal. In reality, the relative position between the start of the synchronization window and the first rising edge of the bucket indication signal is random. In order to guarantee the correct selection of the rising edge of the bucket indication signal at both the source and target synchrotrons, the start of the synchronization window will be rectified to half the period of the bucket indication signal before the selected rising edge. This time is called the ``best estimate of the start of the synchronization window``, denoted as \gls{symb:win_start_rect}. However, the actual start of the synchronization window is impossible to be exactly at \gls{symb:win_start_rect} because of the propogation of the uncertainty. The start of the synchronization window lies between \gls{symb:win_start_rect}$-\delta t_\mathit{w\_start\_rect}$ and $t_\mathit{align}+\delta t_\mathit{w\_start\_rect}$, where \gls{symb:probable_win_start} is the uncertainty of the start of the synchronization window. 

The rf phase of the Reference RF Signal is $\psi_{X}$ at $t_{\psi}^X$. We could calculate the rf phase \gls{symb:phase_s_alignment} of the revolution frequency at the start of the probable rang of alignment, $t_\mathit{align}$-$\delta t_\mathit{align}$.
