...
\begin{equation}
Accuracy=\frac{\SI{1.017}{us}-\SI{20}{ns} \cdot 2-\SI{2}{ns} \cdot 2}{2}\approx  \SI{485}{\ns}\label{accu}
\end{equation}
...
\begin{equation}
	FER=(BER-n\cdot10^{-12})\cdot 880
\end{equation}
where 880 is the number of bits of a B2B Broadcast or B2B Unicast frame and $n$ represents the number of the fiber connection to the Xena receiver ports. In this test, $n=1$ for the B2B Unicast traffic and $n=17$ for the B2B Broadcast traffic. 
...
The requirements for the B2B Broadcast and Unicast traffic of the test are summarized in Tab.~\ref{requirement_network}. In reality, some forward error correction mechanisms will be used for the WR network, which will reduce the FER to $10^{-15}$~\cite{prados_white_2011}.
...
%   \caption{Frame error rate of the B2B Broadcast frames.}
%   \label{packet_loss}
%\end{figure}
...
For the B2B Unicast frames, the 4 WR switches network has approximate \SI{23}{\us} maximum frame transfer latency.
\end{itemize}
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | }
    \hline
     \tabincell{c}{} & \tabincell{c}{FER} & \tabincell{c}{misordered frame} &\tabincell{c}{maximum frame \\transfer latency} \\ \hline
       \tabincell{c}{B2B Broadcast} &  $7.1\cdot10^{-8}$ & 0 & \SI{30}{\us}/switch \\ \hline
		\tabincell{c}{B2B Unicast} 	 & $0$ & 0 & \tabincell{c}{\SI{23}{\us}/4\_switches}\\ \hline
    \end{tabular}
\end{center}
\end{table}

The FER of the B2B Unicast frames meets the requirement. But the FER of the B2B Broadcast frames doesn't meet the requirement. The firmware of the WR switch is still under development by CERN. In actual operation of the B2B transfer, the DM will be informed by the B2B source SCU in the format of the timing event \verb|TGM_B2B_ERROR|, when the B2B Broadcast frame transferred between B2B related SCUs is lost. 

For the B2B transfer system, the upper bound latency of the frames on the WR network is \SI{500}{\us}, see Tab.\ref{requirement_network}. The latency measured in this test does not include the SCU sending and receiving time, which is around \SI{100}{\us}. According to the test result, the maximum frame transfer latency for each WR layer is \SI{30}{\micro\second}. The latency of the WR network is decided by the layers of WR switches and the length of the optical fiber. The propagation of light through the core of an optical fiber is roughly about \SI{200}{\meter/\us}~\cite{noauthor_calculating_2012} and the longest distance in the FAIR campus is around \SI{2}{\kilo\meter}, so the latency of a \SI{2}{\kilo\meter} optical fiber is about \SI{10}{\us}. The layer of WR switches plays a more important role in the latency. 

The tolerable layer of the WR switch between the B2B related \gls{SCU}s is 
		\begin{equation}
		\begin{aligned}
...
\begin{figure}[H]
   ...
   \caption{A possible firing delay between kicker magnets in two tanks of SIS18 extraction kicker.}
	\caption*{\textsl{\small{The bunch is firstly kicked by kicker magnets in the $1^{st}$ tank and than kicked by the kicker magnets in the $2^{nd}$ tanks to the transfer line. The yellow and red ellipse represents the position of the bunches, when the kicker magnets in the $1^{st}$ and $2^{nd}$ tank are fired. The number in the ellipse is used to tell different bunches. The head of the bunch is at the right side. The bunch 2 is firstly kicked. }}}
   \label{kicker_18_1}
\end{figure}
...
Here we take three ion beams, $H^+, U^{28+}$ and $U^{73+}$, to check the maximum and minimum firing delay for kicker magnets in two tanks, because the boundary ion species have the most stringent requirements. %The constant firing delay is determined primarily by the boundary delay range from $H^+, U^{28+}$ and $U^{73+}$ beams, the delay range for other heavy ion species beams must be contained in these boundary range.  
Tab. ~\ref{kicker_delay} shows the longest and shortest firing delay for three ion beams. 
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{The firing delay for SIS18 kicker magnets in two tanks}
\label{kicker_delay}
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | }
    \hline
    Beam & $\beta$ &  \tabincell{c}{bunch gap \\ $t_\mathit{gap}$ } & \tabincell{c}{minimum delay \\ $\frac{d_\mathit{tank1L-tank2R}}{\beta c}$-($t_\mathit{gap}$-\SI{90}{ns})} & \tabincell{c}{maximum delay \\ $t_\mathit{gap}$+($\frac{d_\mathit{tank1R-tank2L}}{\beta c}$-\SI{90}{ns})}\\ \hline
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c  |}
    \hline
    Beam & $\beta$ &  \tabincell{c}{kicker passing\\ time $\frac{d_\mathit{tankL-tankR}}{\beta c}$} & \tabincell{c}{Rise time}& \tabincell{c}{Sum \\ $\frac{d_\mathit{tankL-tankR}}{\beta c}+\SI{130}{ns}$} & \tabincell{c}{bunch gap \\ $t_\mathit{gap}$}\\ \hline

\multicolumn{6}{|c|}{$t_\mathit{gap}=25\%/f_\mathit{rf}^\mathit{SIS100}$ }	 \\ \hline
    $H^+$     & 0.982 & \SI{3}{ns}  	&  $<\SI{130}{ns}$ & \SI{133}{ns} 	& \SI{184}{ns}  \\ \hline
    $U^{28+}$  & 0.568 & \SI{22}{ns} 	&  $<\SI{130}{ns}$ & \SI{152}{ns}  	& \SI{159}{ns}  \\ \hline
    $U^{73+}$ & 0.872 & \SI{15}{ns} 	&  $<\SI{130}{ns}$ & \SI{145}{ns}	& \SI{104}{ns} \\ \hline
\multicolumn{6}{|c|}{$t_\mathit{gap}=50\%/f_\mathit{rf}^\mathit{SIS100}$ }	 \\ \hline
    $H^+$     & 0.982 & \SI{3}{ns}  	&  $<\SI{130}{ns}$ & \SI{133}{ns} 	& \SI{368}{ns}  \\ \hline
    $U^{28+}$  & 0.568 & \SI{22}{ns} 	&  $<\SI{130}{ns}$ & \SI{152}{ns}  	& \SI{318}{ns}  \\ \hline
    $U^{73+}$ & 0.872 & \SI{15}{ns} 	&  $<\SI{130}{ns}$ & \SI{145}{ns}	& \SI{208}{ns} \\ \hline
    \end{tabular}
\end{center}
\end{table}

Tab. ~\ref{kicker_SIS100} shows that the sum of the kicker passing time and rise time is not always shorter than the bunch gap when the bunch gap is $25\%$ of the cavity rf period. Hence, the SIS100 kicker magnets can be fired instantaneous when bunches are longitudinally compressed, e.g. the bunch gap is $50\%$ of the cavity rf period. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
According to the test result, the test setup achieves the functionality of the data collection, data calculation and data redistribution for the FAIR B2B transfer system.
