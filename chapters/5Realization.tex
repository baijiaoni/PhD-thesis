...
The reasonable bucket size of a running bucket is larger than $80\%$ of that of a stationary bucket, namely the bucket area factor $\alpha_b(\phi_{s})\ge 80\%$ (see Chap. ~\ref{background}). Due to the constraint of the bucket size, the synchronous phase must stay within the range between $-6.4^\circ$ and $+6.4^\circ$.
...
\label{wr_network}
This section is part of the results of the characterization of the WR Network of the FAIR General Machine Timing System. The scenario and test definition is done by Cesar Prados as part of his PhD thesis~\cite{prados_notitle_nodate}. Here the tests for the B2B scenario are discussed, evaluated and documented. 
