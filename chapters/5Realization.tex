...
The reasonable bucket size of a running bucket is larger than $80\%$ of that of a stationary bucket, namely $\alpha_b(\phi_{s})\ge 80\%$. Due to the constraint of the bucket size, the synchronous phase must be within the range between $-6.4^\circ$ and $+6.4^\circ$.
...
\begin{center}
    ...
    $\Delta p/p_\mathit{\_max}$ & $\Delta L/L_\mathit{\_max}$ & $\alpha_b(\phi_{s})_\mathit{\_min}$ & $\phi_\mathit{s\_max}$  \\ \hline
       $\pm0.008$	& \tabincell{c}{$H^{+}$ $\pm0.8\cdot10^{-4}$\\$U^\mathit{28+}$ $\pm2.4\cdot10^{-4}$}  &  $80\%$ & $\pm6.4^\circ$ \\ \hline
...
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{The maximum orbit length displacement of three cases}
...
\begin{center}
    ...
    \hline
     \tabincell{c}{Relative\\momentum\\ shift} & \tabincell{c}{Maximum \\orbit length\\ displacement} &\tabincell{c}{Bucket\\size} & \tabincell{c}{Synchronous\\phase} &\tabincell{c}{Adiabaticity}  \\ \hline
       $<4.1\cdot10^{-3}$	 &$<4.09\cdot10^{-5}$ & $>27\%$ &  $\pm34.7^\circ$  & $<0.04$\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{center}
    ...
    \hline
     \tabincell{c}{Relative\\momentum\\ shift} & \tabincell{c}{Maximum \\orbit length\\ displacement} &\tabincell{c}{Bucket\\size}  & \tabincell{c}{Synchronous\\phase} &\tabincell{c}{Adiabaticity}  \\ \hline
       $<3.9\cdot10^{-4}$ & $<3.9\cdot10^{-6}$	& $>86\%$ &  $\pm4.2^\circ$  & $<1.9\cdot10^{-4}$\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
The uncertainty of the extrapolated phase is composed of two parts, the uncertainty of the phase measurement and the uncertainty of the BuTiS T0 incidents. The uncertainty of the extrapolated phase (denoted as \gls{symb:un_h1phase100}) in the phase domain and that in the time domain is calculated as 
\begin{equation} 
...
This section is part of the results of the characterization of the WR Network of the FAIR General Machine Timing System. The scenario and test definition is done by C. Prados as part of his PhD thesis~\cite{prados_wr_nodate}. I executed the test for the B2B scenario and discussed, evaluated and documented the results with C. Prados~\cite{prados_testing_2016}. The test is used to identify the tolerable layers of WR switch for the B2B transfer, including the layers between the B2B related SCUs and the layers between the B2B related SCUs and DM. We use a network analyzer, the Xena traffic generator\footnote{\url{http://xenanetworks.com/layer-2-3-platform/}}, to characterize the properties of the WR network. The Xena traffic generator offers a new class of professional Layer 2-3 Gigabit Ethernet test platform. It sends traffic streams with a unique stream ID and receives the identical traffic streams for identifying the measurement. It can be used for the following measurements.
\begin{itemize}
%    \item \gls{glos:lost_error} - the difference between the number of the sent frames and that of the received frames. In the following analysis, the frame error rate (FER) is used to assess the quality of network, which is defined as the ratio between the number of lost frames and the total number of transferred frames. Due to the transmission channel noise, interference, attenuation and etc, the optical fiber causes bit errors. If a switch receives a frame with one error bit, the complete frame will be discarded. The bit error rate (BER) is used as an approximate estimate of the bit error probability, which is defined as the ratio between the number of bit errors and the total number of transferred bits. For the calculation of the BER, please see ~\cite{prados_white_2011}. In the test, no forward error correction mechanism is used and the bit errors caused by fibers connected to the Xena receiver ports will not cause frame loss, so the relation between the FER and BER is
    \item \gls{glos:lost_error} - the difference between the number of the sent frames and that of the received frames. In the following analysis, the frame error rate (\gls{FER}) is used to assess the quality of network, which is defined as the ratio between the number of lost frames and the total number of sent frames. 
...
For the measurement reported here, the following types of traffic are considered~\cite{prados_testing_2016}.
...
The B2B source SCU sends two \gls{glos:timing_frame}s upwards to the DM within \SI{10}{\ms} for each cycle. Every supercycle contains four cycles. The maximum supercycle repetition frequency for FAIR is the repetition frequency of the $U^{28+}$ supercycle, \SI{2.82}{\Hz}~\cite{liebermann_sis100_2013}. Hence, the bandwidth is $2.82\cdot4\cdot2\cdot880<$ \SI{20}{kbit/s}. 
	\item B2B Broadcast

Maximum 10 B2B broadcast timing frames are sent within \SI{10}{\ms} for each cycle. Hence, the bandwidth of the B2B broadcast traffic is $2.82\cdot4\cdot10\cdot880<$ \SI{100}{kbit/s}.
...
For the WR network for FAIR, three VLANs with different priorities are applied according to the importance of the traffic. The DM Broadcast, DM Unicast and B2B Unicast are assigned to the VLAN 7 with the highest priority. The B2B Broadcast is assigned to the VLAN 6 with the secondary high priority and the Management Traffic is assigned to the VLAN 5 with the lowest priority~\cite{prados_testing_2016}.
...
\subsubsection{Measurement of Lost Frame}
The test is running for 45 days. 
\begin{itemize}
    \item Lost Frame of the B2B Unicast traffic

The frame loss of the B2B Unicast traffic is measured from the port 17 to the port 1. The B2B Unicast traffic has no lost frame. Besides, there is no misordered frame.

    \item Lost frame of the B2B Broadcast traffic

The frame loss of the B2B Broadcast traffic is measured separately from the port 16 to other ports. There are two lost frames individually from the port 16 to the port 11, 12, 17 and 18 and their individual FER is $1.78\cdot 10^{-8}$. Fig. ~\ref{packet_loss} shows the FER for B2B Broadcast traffic. For the B2B Broadcast frames, there is no misordered frame.
...
\subsubsection{Measurement of Frame Transfer Latency}

\begin{itemize}
    \item Maximum frame transfer latency of the B2B Broadcast frames

For the B2B Broadcast frames, the frame transfer latency of the stream from the port 16 to other ports are measured. Fig. ~\ref{Max_latency_jitter} and Tab. \ref{max latency jitter} show the test result for the maximum frame transfer latency of the B2B Broadcast frames.   
%		\begin{itemize}
...
\begin{figure}[H]
   ...
   \caption{The maximum frame transfer latency for B2B Broadcast frames.}
   \label{Max_latency_jitter}
\end{figure}
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{The maximum frame transfer latency of the B2B Broadcast frames}
...
Every WR switch causes a maximum \SI{30}{\us} latency for the B2B Broadcast traffic.

    \item Maximum frame transfer latency for the B2B Unicast frames

For the B2B Unicast frames, the maximum frame transfer latency of the stream from the port 17 to the port 1 is measured, see Fig. ~\ref{Max_latency_jitter_unicast}. 
...
For the B2B Unicast frames, the 4 layer WR switches has approximately \SI{23}{\us} maximum frame transfer latency.
\end{itemize}
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | }
    \hline
     \tabincell{c}{} & lost frame &\tabincell{c}{FER} & \tabincell{c}{misordered\\ frame} &\tabincell{c}{maximum frame \\transfer latency} \\ \hline
       \tabincell{c}{B2B Broadcast} & $8/45\_days$ & $7.1\cdot10^{-8}/45\_days$ & 0 & \SI{30}{\us}/switch \\ \hline
		\tabincell{c}{B2B Unicast} 	 & $0/45\_days$ & $0/45\_days$ & 0 & \tabincell{c}{\SI{23}{\us}/4\_switches}\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
Due to the transmission channel noise, interference, attenuation and other factors, the fiber connection causes bit errors. The bit error rate (\gls{BER}) is defined as the ratio between the number of bit errors and the total number of sent bits. The BER for fiber connections is calculated as~\cite{prados_white_2011} 
\begin{equation}
	BER=n\cdot10^{-12}
\end{equation}
where n is the number of the fiber connections used to establish the communication and the BER of every fiber connection is $10^{-12}$.
...
\end{equation}
where 880 is the number of bits of a B2B Broadcast or B2B Unicast frame and $m$ represents the number of the fiber connections to the frame receive FECs and $n-m$ is the layers of WR switch.  

In actual application of the B2B transfer system, there is only one frame receive FEC for the B2B Unicast traffic, namely $m=1$. For the B2B Broadcast traffic, there are total six ring accelerators (SIS18, SIS100, ESR, CR, HESR and CRYRING) and every ring needs three FECs for the data exchange within the VLAN 6, the B2B source/target SCU, the Trigger SCU and the Kicker SCU. Hence, there is one frame send FEC and 17 frame receive FECs, namely $m=17$. In the test setup, the Xena ports are used as FECs. We have $m=1$ for the B2B Unicast traffic and $m=17$ for the B2B Broadcast traffic by coincidence. Fig. ~\ref{FER_2case} shows the relation between the FER and the number of fiber connection for two traffics.   
\begin{figure}[H]
   \centering   
...
In the test, n equals to 5 for the B2B Unicast traffic and n equals to 21 for the B2B Broadcast. The corresponding FER is $0.35\cdot10^{-8}$ (see Fig. ~\ref{FER_2case}). According to the test result, the FER of the B2B Broadcast frames is $7.1\cdot10^{-8}$, which is larger than $0.35\cdot10^{-8}$, so it doesn't meet the requirement. The firmware of the WR switch is still under development by CERN. We can not verify whether the FER of the B2B Unicast frames meets the requirement or not based on the 45 days test result. A long term test should be done for the verification. 
...
From the perspective of the latency, the tolerable layer of the WR switch between the B2B related \gls{SCU}s and DM is
		\begin{equation}
		\begin{aligned}
...
The more layers of WR switch, the more fiber connections and the higher corresponding FER. The tolerable FER of the B2B transfer system is out of the scope of the dissertation. Here we assume that one lost frame is acceptable per month. In this case, maximum 18 layers of WR switch are used between the B2B related SCUs and DM and maximum 4 layers of WR switch are used between the B2B related SCUs. If one lost frame is acceptable per year, 1 WR switche is used between the B2B related SCUs and DM and null WR switch is used between the B2B related SCUs. In this case, some forward error correction mechanism must be used to reduce the FER. The FER is reduced to the $10^{-15}$ order of magnitude when specific forward error correction mechanisms are used for the WR network ~\cite{prados_white_2011} and the layer of WR switch is determined by the transfer latency.

In the actual operation of the B2B transfer, the DM will be informed by the B2B source SCU in the format of the timing event \verb|TGM_B2B_ERROR|, when the B2B Broadcast frame is lost. For more details, please see Sec. ~\ref{sec_firmware}.


\subsubsection{Conclusion}
The tolerable layers of WR switch depends not only on the upper bound transfer latency, but also the tolerable FER of the B2B transfer system. The tolerable FER of the B2B transfer system is out of the scope of the dissertation. 

If no forward error correction mechanism is used for the B2B network, the layers of WR switch is mainly decided by the tolerable FER. e.g. one lost frame is acceptable per month. The maximum 18 layers of WR switch are used between the B2B related SCUs and DM and maximum 4 layers of WR switch are used between the B2B related SCUs.


If specific forward error correction mechanisms are used for the B2B network and the FER is $10^{-15}$ ~\cite{prados_white_2011}, the layers of WR switch is mainly decided by the tolerable latency. Hence, the tolerable layers of WR is 67 between the B2B related \gls{SCU}s and DM and the tolerable layer of WR switch is 13 between the B2B related \gls{SCU}s.    
...
\label{real_kicker}
The SIS18 extraction kicker consists of nine kicker magnets. In the existing topology, five kicker magnets are evenly distributed in the $1^{st}$ tank and the other four kicker magnets are evenly distributed in the $2^{nd}$ tank. The investigation is based on the assumption that the kicker magnets in one tank are controlled by a common kicker control electronics, which receives a trigger signal from a common TD module. The nine kicker magnets can also be individually controlled by their own kicker control electronics and TD module. The implementation is out of the scope of this dissertation. The kicker magnets in a common tank are triggered by trigger signals with a local delay compensation for the electronics, the length of the energy transmission cable and etc. Fig. ~\ref{SIS18_kicker} shows the schematic diagram of the kicker magnets in the SIS18 $2^{nd}$ tank. The width of each kicker magnet is \SI{0.25}{m} and the distance between two kicker magnets is \SI{0.09}{m}. The distance between the two tanks is \SI{19.17}{m} ~\cite{ros_sis18_2008}. 
%kicker.pptx
\begin{figure}[H]
   \centering   
...
The SIS100 injection kicker consists of six kicker magnets, which are evenly distributed in a common tank. The width of each kicker magnet is \SI{0.22}{m} and the distance between two magnets is \SI{0.23}{m}. For the B2B transfer, the rise time of the SIS18 extraction kicker and the rise time of the SIS100 injection kicker are \SI{90}{ns} and \SI{130}{ns} ~\cite{blell_detailed_2014}, which is denoted by \gls{symb:kicker_rise}.  The kicker rise time must fit within the bunch gap, e.g. the bunch gap is 25$\%$ of the cavity rf period ~\cite{blell_injection_2014, liebermann_sis100_2013}. The bunch gap is denoted by \gls{symb:G}.  All the analysis in this section dose not take the jitter of the kicker trigger signal into consideration (approximately \SI{1}{ns}). 
...
Three ion beams, $H^+, U^{28+}$ and $U^{73+}$, are used to check the instantaneous trigger of kicker magnets in a common tank, because the boundary ion species have the most stringent requirements. Tab. ~\ref{kicker_SIS18} shows the kicker passing time, the rise time and the bunch gap for $H^+$, $U^{28+}$ and $U^{73+}$ beams. 
...
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Parameters related to the simultaneous trigger of the SIS18 extraction kicker magnets in a common tank}
\label{kicker_SIS18}
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | c | c |}
    \hline
...
\begin{figure}[H]
   ...
   \caption{A possible triggering delay between extraction kicker magnets in the SIS18 two tanks.}
	\caption*{\textsl{\small{The bunch is firstly kicked by kicker magnets in the $1^{st}$ tank and than kicked by the kicker magnets in the $2^{nd}$ tanks to the transfer line. The yellow and red ellipse represents the position of the bunches, when the kicker magnets in the $1^{st}$ and $2^{nd}$ tank are triggered. The number in the ellipse is used to tell different bunches. The head of the bunch is at the right side. The bunch 2 is firstly kicked. }}}
   \label{kicker_18_1}
\end{figure}
Fig.~\ref{kicker_18_1} shows a possible triggering delay between kicker magnets in the two tanks. \gls{symb:d} denotes the distance between two tanks, which equals to \SI{19.17}{m}. \gls{symb:L} denotes the distance from the leftmost to the rightmost kicker magnets, which equals to \SI{22.05}{m} = $d_\mathit{tank1R-tank2L} + 9\cdot \SI{0.25}{m} + 7\cdot \SI{0.09}{m}$. \gls{symb:D} denotes the distance between the rightmost of the $1^{st}$ tank to the rightmost of the $2^{nd}$ tank, which equals to \SI{20.44}{m} = $d_\mathit{tank1R-tank2L} + 4\cdot \SI{0.25}{m} + 3\cdot \SI{0.09}{m}$. The kicker magnets in the $1^{st}$ tank are triggered simultaneously when the tail of the bunch 1 passes by the $1^{st}$ tank completely. The kicker magnets in the $2^{nd}$ tank are triggered simultaneously when the tail of the bunch 1 passes by the $2^{nd}$ tank completely. The delay for the trigger two tanks in this scenario is $d_\mathit{tank1R-tank2R}/\beta c$. 
\begin{figure}[H]
   \centering   
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Test Setup in the Timing Aspects}
\label{real_test}

From the B2B systematically functional perspective, the test setup achieves that the B2B source SCU collects data from the B2B target SCU, that the B2B source SCU calculates the start of the synchronization window and that the B2B source SCU distributes the start of the synchronization window to the Trigger SCU. Besides, the test setup is used to check the running time of the firmware of the B2B related SCUs to verify that the firmware running on the soft CPU, LM32, meets the time constraints of the B2B transfer system, see Sec. ~\ref{sec:time_constraint}.  In Sec. ~\ref{sec:test_timing} the test setup will be introduced. The procedure and the flowchart of the firmware will be explained in Sec. ~\ref{sec:proc}. Sec. ~\ref{sec:function_result} and Sec. ~\ref{sec:execution} show the functional test result and the measurement of the firmware execution time. Whether the execution time of the firmware meets the time constraints of the B2B transfer system or not will be discussed in Sec. ~\ref{result_dis}.  
...
%	 \tabincell{c}{1. Login\\
%					  2. Use the saftlib \footnote{\url{https://www-acc.gsi.de/wiki/Timing/TimingSystemDocumentsSaftlib}} library function ``saft-ecpu-ctl`` to configure the ECA queue \footnote{\url{https://www-acc.gsi.de/wiki/Timing/TimingSystemHowTriggerLM32FromEca}} of the B2B source SCU to storage B2B frames \verb|CMD_B2B_START| and \verb|TGM_PHASE_TIME|. \\
...
%\item After receiving \verb|CMD_B2B_START|, both the firmware of the B2B source and target SCUs collect the timestamp of the rising edge of the TTL signals of the SIS18 and the SIS100 DS345s. 
%\item The firmware of the B2B source SCU reads the phase value from the SCU slave, which works as the PAP module. 
%\item The firmware of the B2B target SCU transfers the frame \verb|TGM_PHASE_TIME| containing the phase and the corresponding timestamp to the B2B source SCU.
...
%\item The firmware of the B2B source SCU sends the frame \verb|TGM_SYNCH_WIN| containing the start timestamp of the synchronization window to the WR network.
...
For more details of the configuration, please see Appendix ~\ref{appendix_conf}. The flowchart of the test setup firmware of the B2B source SCU and that of the B2B target SCU and that of the Trigger SCU are shown in Fig. ~\ref{test_flwochart}, Fig. ~\ref{test_flwochart1} and Fig. ~\ref{test_flwochart2}. The firmware are kept in Git repository \footnote{\url{https://github.com/GSI-CS-CO/bel_projects/tree/lm32_B2B_merge}}. The firmware of the test setup realizes partial function of the firmware in Sec. ~\ref{sec_firmware}.
\begin{figure}[H]
   \centering   
...
\subsection{Measurement of Execution Time}
\label{sec:execution}
During the test, the SignalTap II Logic Analyzer is used to count the needed system clocks for the execution processes of the firmware, see Tab. ~\ref{execution_time}. The LM32 is connected to other intellectual property cores by the System-on-Chip bus. The test result is based on the precondition that the related System-on-Chip bus is not occupied by any other applications, when the firmware is running.

\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{The running time of the execution processes of the B2B source SCU firmware}
\label{execution_time}
\begin{center}
    \begin{tabular}{ | c |c | c | c | c | c | c | c |}
    \hline
     execution process & \tabincell{c}{average \\running time} & \tabincell{c}{worst \\running time}  \\ \hline

      \tabincell{c}{LM32 polls to check the reception \\of CMD\_B2B\_START after ECA\\ queue stores CMD\_B2B\_START} & \SI{336}{\ns} & \SI{540}{\ns}\\ \hline
      LM32 reads from/writes to SCU slave & \SI{450}{\ns} & \SI{650}{\ns}\\ \hline
\tabincell{c}{LM32 polls to check the reception \\of TGM\_PHASE\_TIME after ECA\\ queue stores TGM\_PHASE\_TIME}& \SI{336}{\ns} & \SI{540}{\ns}\\ \hline
      \tabincell{c}{LM32 reads phase and timestamp \\from TGM\_PHASE\_TIME} & \SI{2.7}{\us} & \SI{2.9}{\us}\\ \hline
      LM32 calculates synchronization window & \SI{12.6}{\us}& \SI{12.8}{\us} \\ \hline
      LM32 calculates phase correction value & \SI{2.0}{\us}& \SI{2.2}{\us} \\ \hline
      LM32 sends TGM\_SYNCH\_WIN to WR network & \SI{3.2}{\us} & \SI{3.4}{\us}\\ \hline
...
We use the measurement result from the last section to analyze whether the final firmware meets the time constraint of the B2B transfer system or not.

After the ECA queue stores \verb|CMD_B2B_START|, the firmware of the B2B source SCU needs maximal \SI{1.2}{\us} (\SI{540}{\ns}+\SI{650}{\ns}) to inform the PAP module to start the phase extrapolation. Compared with \SI{500}{\us} phase extrapolation duration,  \SI{1.2}{\us} is negligible. \SI{500}{\us} after the execution of \verb|CMD_B2B_START|, the firmware needs maximal \SI{1.2}{\us} to read the extrapolated phase from the PAP module, \SI{1.2}{\us} is much shorter than one BuTiS T0 \SI{100}{kHz} period, which guarantees the correct extrapolated phase is read. 

After the ECA queue stores \verb|TGM_PHASE_TIME|, the firmware of the B2B source SCU needs maximal \SI{3.5}{\us} (\SI{540}{\ns}+\SI{2.9}{\us}) to get the phase and timestamp. The calculation running time is maximal \SI{15}{\us} (\SI{12.8}{\us}+\SI{2.2}{\us}) and the sending time of \verb|TGM_SYNCH_WIN| is \SI{3.4}{\us}. The sum of these execution time is shorter than \SI{100}{\us}, which is negligible compared with \SI{10}{\ms}.

After the ECA queue stores \verb|TGM_PHASE_CORRECTION|, the Trigger SCU needs maximal \SI{4.2}{\us} (\SI{3.5}{\us}+\SI{650}{\ns}) to transfer the phase correction value to the PCM. \SI{4.2}{\us} is shorter than one BuTiS T0 \SI{100}{kHz} period, which guarantees the trigger of the phase correction at the correct BuTiS T0 incident.

In conclusion, the test setup achieves the systematic functionality, the data collection, calculation and distribution. Besides, it proves that the firmware on the LM32 meets the time constraints of the B2B transfer system, when the firmware is running and the related System-on-Chip bus is not occupied by any other applications. 
