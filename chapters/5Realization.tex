...
\section{Beam dynamic analysis of two synchronization methods for the $U^{28+}$ B2B transfer from the SIS18 to the SIS100}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Analysis for the phase shift method}
...
\begin{itemize}
	\item A maximum frequency offset for the rf frequency modulation exists.
	\item $\dot{\Delta f_{\mathit{rf}}}$ must exist and must be small enough, namely $\Delta f_{rf}$ must be continuous and the change of $\Delta f_{rf}$ must be slow enough.
	\item $\ddot{\Delta f_{\mathit{rf}}}$ must exist and must be small enough. Namely, $\dot{\Delta f_{\mathit{rf}}}$ must be continuous and the change of $\dot{\Delta f_{\mathit{rf}}}$ must be slow enough. 
\end{itemize}

According to these criterion, some rf frequency modulation cases are excluded. e.g. a trapezoid modulation and a triangular modulation, whose first derivative are not continuous. The following three examples of rf frequency modulation comply with the above criterion. Case (1) is a sinusoidal modulation, case (2) is a parabolic modulation, which consists of three parabolas and two lines between every two parabolas. Case (3) is also a parabolic modulation, including of three parabolas. The phase shift is assumed to be achieved within \SI{7}{ms}. Three rf frequency modulation cases are shown in Fig.~\ref{4case}. All three cases give the same phase shift, $\Delta \phi=\pi$, which is proved by substituting each form of $\Delta f_{rf}(t)$ into eq.~\ref{phase_integration} and performing integration. 
...
\end{center}
\end{table}
Tab. \ref{momentum excursion} shows the maximum relative momentum shift and the time for three cases. The maximum SIS18 tolerable relative momentum shift is $\pm 0.008$. For all cases, the maximum relative momentum shift is within the acceptable range. Hence, all cases are applicable. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\item Synchronous phase

The rf frequency modulations make the synchronous phase deviate from the nominal value $0^\circ$. Fig.~\ref{synch_phase} shows the changes in the synchronous phase $\phi_s$(t). It is calculated by substituting values into eq.~\ref{deriva_voltage1}. For all three cases, the synchronous phase $\Delta \phi_s(t)$ during the modulations are continuous without any phase jumps. Hence, all cases are applicable.
\begin{figure}[H]
   \centering   
...
Tab. ~\ref{adiabaticity_param} shows the maximum adiabaticity parameter for three cases. For case (1), the maximum of $\varepsilon$ is 0.000030. For case (2), the maximum of $\varepsilon$ occurs at $1/6T$, $2/6T$, $4/6T$ and $5/6T$. From Fig.~\ref{synch_phase}, we could see the change of the synchronous phase $\dot{\phi_s(t)}$ at these time points is big. For case (3), the maximum of $\varepsilon$ occurs at $1/4T$ and $3/4T$, when the change of the synchronous phase $\dot{\phi_s(t)}$ is big. For all three cases, the adiabaticity parameter has the order of magnitude of $-5$, so all three cases are applicable. 
...
For SIS18, the chromaticity $Q^`_x$ and $Q^`_y$ is $4.17$ and $3.4$. Substituting chromaticity and maximum momentum shift (see. Tab. \ref{momentum excursion}) into eq. ~\ref{eq:chromaticity_x}. The chromatic \gls{glos:tune} shift $\Delta Q_x$ and $\Delta Q_y$ during rf modulations for three cases can be calculated. Beacuse case (1), case (2) and case (3) have same maximum relative momentum shift, the chromatic tune shift is same for three rf frequency modulations.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Analysis for the frequency beating method}
...
\subsubsection{Longitudinal dynamics analysis of the frequency detune of the SIS18}
For the frequency beating method, the rf frequency detune is done directly after the rf ramp. The accepted excursion of the SIS18 orbit is 8mm ~\cite{liebermann_fair_2013}, namely 
\begin{equation}
\frac{\Delta{R}}{R} = \pm 2.4 \times 10^{-4}
%\gls{symb:radius} = \pm 2.4 \cdot 10^{-4}
\end{equation}
From eq. ~\ref{eq:eq4} and eq. ~\ref{eq:eq5}, the tolerate rf frequency and the magnetic field change for $U^{28+}$ at the extraction energy \SI{200}{MeV/u} are
\begin{equation}
...
where the maximum rf frequency detune approximates to \SI{370}{Hz} for the cavity rf frequency of \SI{1.57}{MHz} of $U^{ 28+}$. Fig.~\ref{sis18_ramp} shows the rf frequency derivation during the rf ramp. In the simulation, it is assumed that the rf frequency is detuned at \SI{0.2756}{s} with \SI{6.08}{Hz/us}, see blue rectangle in Fig.~\ref{sis18_ramp}. For the sake of simplicity, \SI{200}{Hz} is used as the rf frequency detune. The SIS18 needs approximate \SI{33}{us} to reach 200 Hz with \SI{6.08}{Hz/us}.
\begin{figure}[!htb]
   \centering   
...
The B2B source SCU sends two \gls{glos:timing_frame}s upwards to the DM within \SI{10}{\ms} for each cycle. Every supercycle contains four cycles. The maximum supercycle repetition frequency for FAIR is the repetition frequency of the $U^{28+}$ supercycle, \SI{2.82}{\Hz}. The bandwidth is $2.82\times4\times2\times880\approx$ \SI{20}{kbit/s}. 
	\item B2B Broadcast

Maximum 10 B2B broadcast timing frames are sent within \SI{10}{\ms} for each cycle. So the bandwidth is $2.82\times4\times10\times880\approx$ \SI{100}{kbit/s}.
...
The frame loss rate of the stream from port 17 to port 1 is measured for the B2B Unicast frames. The frame loss rate of the stream from port 16 to other ports is measured for the B2B Broadcast frame. The B2B Unicast frames has no frame loss. For the B2B Broadcast frames, the frame loss rate from port 16 to port 11, 12, 17 and 18 is $1.78\times 10^{-8}$, see Fig. ~\ref{packet_loss}. 

\begin{figure}[H]
   \centering   
   \includegraphics*[width=160mm]{packet_loss.png}
   \caption{The frame loss rate for B2B Broadcast frames}
   \label{packet_loss}
\end{figure}
...
For the B2B Broadcast frames, the latency and jitter of the stream from port 16 to other ports is measured. 
		\begin{itemize}
...
For the B2B unicast frames, the latency and jitter of the stream from port 17 to port 1 are measured. 
