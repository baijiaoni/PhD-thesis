...
\section{Phase match}
\label{match}
The rf voltage of two rf systems of the large and small synchrotrons, $V^1$ and $V^s$, are
\begin{equation}
\label{rf_freq1}
	V^1=V_0\sin(2\pi f_\mathit{rf}^{l}t+\theta_\mathit{1})
\end{equation}
\begin{equation}
\label{rf_freq2}
...
Only $x$ is a variable in eq. ~\ref{phase_diff_cir_int}, which is marked in red color. From the $1^{\mathit{st}}$ item in eq. ~\ref{phase_diff_cir_close_int}, we know that the phase difference $\Delta \phi_{l\_s}$ is constant when $x$ is the integer multiple of the $h^{s}\kappa$. Namely, the rf signal of the large synchrotron has a constant phase every $(h^{s}\kappa)^{\mathit{th}}$ positive zero-crossing of the rf signal of the small synchrotron. The constant phase of the rf system of the large synchrotron happens at the frequency of $f_{\mathit{rf}}^{l}/h^{l}$ and the corresponding postive zero-crossing of the rf system of the small synchrotron at the frequency of $f_{\mathit{rf}}^{s}/(h^{s}\kappa)$. Similarly, we can get these two rf frequencies for the phase difference between the small and large synchrotrons $\Delta \phi_{s\_l}$. Hence, $f_{\mathit{rf}}^{l}/h^{l}$ and $f_{\mathit{rf}}^{s}/(h^{s}\kappa)$ are the synchronization rf frequencies for the phase shift method, namely, $f_{\mathit{syn}}^{l}=f_{\mathit{rf}}^{l}/h^{l}$ and $f_{\mathit{syn}}^{s}=f_{\mathit{rf}}^{s}/(h^{s}\kappa)$. In this scenario, the phase match must be achieved by a phase shift of one of rf systems.
...
Fig. ~\ref{cir_int} illustrates an example with $\kappa=5$, $h^s=1$ and $h^l=10$. So $f_{\mathit{rf}}^{l}=2f_{\mathit{rf}}^{s}$. GCD of $h^l$ and $h^s \cdot \kappa$ is 5. Hence, according to eq. ~\ref{synch_freq1} and eq. ~\ref{synch_freq2}, two synchronization rf frequencies are $f_{\mathit{syn}}^{l}=f_{\mathit{rf}}^{l}/2$ and $f_{\mathit{syn}}^{s}=f_{\mathit{rf}}^{s}$. The constant phase difference $\Delta \phi_{l\_s}$ happens every ${h^l/Y}=2$ rf period of the large synchrotron or every ${(h^s\cdot \kappa)/Y}=1$ rf period of the small synchrotron (red rectangle in Fig. ~\ref{cir_int}). 
%phase_match.dox
\begin{figure}[!htb]
   \centering   
...
\end{equation}
where $\lambda$ is the decimal and in the range of $10^{-3}$. From the circumference ratio, the revolution frequency ratio of two synchrotrons can be calculated.
\begin{equation}
...
Only $g$ is a variable in eq. ~\ref{phase_diff_cir_close_int1}, which is marked in red color. From the $2^{\mathit{nd}}$ item in eq. ~\ref{phase_diff_cir_close_int1}), we know that the phase difference $\Delta \phi_{l\_s}$ increases at the speed of $\frac{h^{l}h^{s}\lambda}{h^{s}\kappa+h^{s}\lambda}\cdot2\pi$ ($h^{l}h^{s}\lambda$ is much smaller than $h^{s}\kappa+h^{s}\lambda$) every the $(h^{s}\kappa)^{\mathit{th}}$ positive zero-crossing of the rf system of the small synchrotron. From the $1^{\mathit{st}}$ item in eq. ~\ref{phase_diff_cir_close_int1}, the phase step growth of the rf system of the large synchrotron happens at the frequency of $f_{\mathit{rf}}^{l}/h^{l}$ and the corresponding postive zero-crossing of the rf system of the small synchrotron at the frequency of $f_{\mathit{rf}}^{s}/(h^{s}\kappa)$. Similarly, we can get these two rf frequencies for the phase difference between the small and large synchrotrons $\Delta \phi_{s\_l}$. Hence, $f_{\mathit{rf}}^{l}/h^{l}$ and $f_{\mathit{rf}}^{s}/(h^{s}\kappa)$ are the synchronization rf frequencies for the frequency beating method, namely, $f_{\mathit{syn}}^{l}=f_{\mathit{rf}}^{l}/h^{l}$ and $f_{\mathit{syn}}^{s}=f_{\mathit{rf}}^{s}/(h^{s}\kappa)$. In this scenario, the rf frequency can be detuned on one of rf systems.
...
\begin{figure}[!htb]
   ...
   \caption{The step grown phase difference between two rf systems when circumference ratio is close to an integer.}{Red rectangles represent step grown phase differences.}
   \label{cir_noint}
\end{figure} 

Fig. ~\ref{cir_noint} illustrates an example.
The beating frequency must not be too large in order to guarantee the precise of the phase match due to the step grown phase, but also not too small to satisfy the time constraint for the phase match. 
...
Only $g$ is a variable in eq. ~\ref{phase_diff_far_fram_int1}, which is marked in red color. From the $2^{\mathit{nd}}$ item in eq. ~\ref{phase_diff_far_fram_int1}, we know that the phase difference $\Delta \phi_{l\_s}$ increase at the speed of $\frac{h^{l}h^{s}\lambda n^2}{h^{s}m+h^{s}\lambda n}\cdot2\pi$ ($h^{l}h^{s}\lambda n^2$ is much smaller than $h^{s}m+h^{s}\lambda n$) every the $(h^{s}m)^{\mathit{th}}$ positive zero-crossing of the rf system of the small synchrotron. From the $1^{\mathit{st}}$ item in eq. ~\ref{phase_diff_far_fram_int1}, we know that the phase step growth  of the rf system of the large synchrotron happens at the frequency of $f_{\mathit{rf}}^{l}/(h^{l}n)$ and the corresponding postive zero-crossing of the rf system of the small synchrotron at the frequency of $f_{\mathit{rf}}^{s}/(h^{s}m)$. Similarly, we can get these two rf frequencies for the phase difference between the small and large synchrotrons $\Delta \phi_{s\_l}$. Hence, $f_{\mathit{rf}}^{l}/(h^{l}n)$ and $f_{\mathit{rf}}^{s}/(h^{s}m)$ are the synchronization rf frequencies for the frequency beating method, namely, $f_{\mathit{rf}}^{l}/(h^{l}n)$ and $f_{\mathit{syn}}^{s}=f_{\mathit{rf}}^{s}/(h^{s}m)$. In this scenario, the rf frequency can be detuned on one of rf systems.
...
Y is the GCD of $h^ln$ and $h^s m$. In this scenario, the phase difference between two rf systems varies periodically. The beating frequency is
\begin{equation} 
\Delta f=|f_{\mathit{syn}}^{l}-f_{\mathit{syn}}^{s}|
\end{equation}
...
\begin{equation}
\frac{\Delta f_{\mathit{rf}}}{f_{\mathit{rf}}} = \alpha_p\cdot{\frac{\Delta{B}}{B}}
\label{eq:eq5}
\end{equation}
\end {itemize}
