...
\begin{figure}[!htb]
   \centering   
   \includegraphics*[width=150mm]{ext_inj_ill.pdf}
...
\end{equation}
where \gls{symb:init_pse} is the initial value of the phase deviation between the synchronization frequency and the synchronization reference signal. $\phi^X(t)$ is within the range between $-\pi$ ang $\pi$, which is determined by the realization of the phase deviation measurement, for more details, please see ``Development of the LLRF system for a deterministic Bunch-to-Bucket transfer for FAIR`` ~\cite{ferrand_development_????}.
\begin{figure}[!htb]
   \centering   
