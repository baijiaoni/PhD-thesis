...
The B2B source SCU obtains the delay compensation for the TOF, all propagation delays, the kicker preparation time and the bucket delay (denoted as \gls{symb:delay_com}) from the SM. It calculates the start of the synchronization window (denoted as \gls{symb:win_start}), taking the delay compensation into consideration and transfers \gls{symb:win_start} to the DM and the source and target Trigger SCUs via the WR network by the timing frame \verb|TGM_SYNCH_WIN|. The Trigger SCUs are used to produce the kicker trigger signals. The DM must transfer \verb|TGM_SYNCH_WIN| further to the BI for the indication of the beam. For the phase shift method, the rf frequency detuning has a fixed duration $T$. The start of the synchronization window for the phase shift method is calculated as
\begin{equation}
t_\mathit{w}=t_\psi^\mathit{X}+\SI{600}{\us}+T-\frac{T_w}{2}-t_\mathit{delay}\label{syn_win_start}
\end{equation}
where \SI{600}{\us} consists of the upper bound transfer delay on the WR network (\SI{500}{\us}) and the B2B source SCU calculation time (\SI{100}{\us}), more details, please see Chap. ~\ref{realization}. $T$ is long enough to guarantee that the BI receives \verb|TGM_SYNCH_WIN| and is activated before the start of the synchronization window. 
...
\end{equation}
where the term of $n\cdot \frac{1}{|f_{\mathit{syn}}^\mathit{src}-f_{\mathit{syn}}^\mathit{trg}|}$ is used to guarantee the activation of the BI timely. 
