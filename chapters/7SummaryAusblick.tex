...
\begin{center}
    ...
    \hline
\rowcolor[gray]{0.5}
     	No. & FAIR use cases & \tabincell{c}{B2B injection \\center mismatch} & Remark \\ \hline
1&	\tabincell{c}{$U^{28+}$ B2B transfer\\ from SIS18 to SIS100}	&	$\pm0.4^\circ$ 	& \multirow{5}{*}{\tabincell{c}{The FAIR use cases 1-5\\ have the B2B injection\\ center mismatch smaller than $\pm1^\circ$,\\ because the circumference\\ ratio between two rings\\ is an integer \\or close to an integer.}}\\ \cline{1-2}
2&	\tabincell{c}{$H^{+}$ B2B transfer\\ from SIS18 to SIS100}	&	$\pm0.4^\circ$	&	\\ \cline{1-3}
3&	\tabincell{c}{h=4 B2B transfer\\ from SIS18 to ESR}				&	$\pm0.5^\circ$	&	\\ \cline{1-3}
4&	\tabincell{c}{h=1 B2B transfer\\ from SIS18 to ESR}				&	$\pm0.5^\circ$	&  \\ \cline{1-3}
5&	\tabincell{c}{B2B transfer \\from ESR to CRYRING}				&	$\pm0.5^\circ$	&	\\ \hline
6&	\tabincell{c}{B2B transfer \\from CR to HESR}	&$\pm1.2^\circ$ & \tabincell{c}{The B2B injection center\\ mismatch is just beyond\\ the specification, but it is\\ still acceptable. Although the \\circumference ratio between two rings \\ is far away from an integer.}\\ \hline

7&	\tabincell{c}{$H^{+}$ B2B transfer \\from SIS100 to CR\\ via the pbar target}			&$\pm41.5^\circ$& \multirow{5}{*}{\tabincell{c}{The B2B injection center mismatch\\ is far beyond the specification, \\because the energy ratio \\before and after targets\\ is arbitrary. (The FAIR use case No. 8\\ is close to the specification\\ and still acceptable by coincidence.)}}\\ \cline{1-3}
8&	\tabincell{c}{RIB B2B transfer\\from SIS100 to CR \\ via the Super FRS}					&	$\pm2.1^\circ$	& \\ \cline{1-3}

9&	\tabincell{c}{RIB B2B transfer \\from SIS18 to ESR \\via the FRS}	&$\pm31.2^\circ$	&		\\ \hline
