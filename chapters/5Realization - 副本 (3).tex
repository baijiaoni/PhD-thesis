...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | c |}
	    \hline
	\tabincell{c}{ FAIR use cases}& $|f_{\mathit{B2B}}^\mathit{trg}-f_{\mathit{ref}}|$ & $\delta \psi^{X}_\mathit{0\_phase}$  & $|f_{\mathit{syn}}^\mathit{src}-f_{\mathit{syn}}^\mathit{trg}|$ & $\delta t_\mathit{align}$ & $T_\mathit{w}$ & $\delta t_\mathit{align}/T_\mathit{w}$\\ \hline
   \tabincell{c}{$U^{28+}$ B2B transfer from the SIS18 to the SIS100}& \SI{43}{kHz} & $0.012^\circ$  &\SI{200}{Hz} & \SI{2.357}{\micro\second} & \SI{6.359}{\micro\second}& 0.37 \\ \hline
	\tabincell{c}{$H^{+}$ B2B transfer from the SIS18 to the SIS100}& \SI{28}{kHz}&$0.011^\circ$  & \SI{200}{Hz} & \SI{1.086}{\micro\second}& \SI{3.678}{\micro\second}& 0.29  \\ \hline
	\tabincell{c}{h=4 B2B transfer from the SIS18 to the ESR}&\SI{14}{kHz}& $0.011^\circ$ &  \SI{1899}{Hz} & \SI{0.046}{\micro\second}& \SI{1.456}{\micro\second} & 0.03 \\ \hline
	\tabincell{c}{h=1 B2B transfer from the SIS18 to the ESR}&\SI{12}{kHz}&$0.010^\circ$ & \SI{1368}{Hz} & \SI{0.029}{\micro\second}& \SI{1.017}{\micro\second} & 0.03\\ \hline	
	\tabincell{c}{B2B transfer from the ESR to the CRYRING}& \SI{14}{kHz} & $0.011^\circ$  & \SI{949}{Hz} &\SI{0.046}{\micro\second}& \SI{1.456}{\micro\second} &0.03 \\ \hline	
	\tabincell{c}{$H^{+}$ B2B transfer from the SIS100 to the CR\\ via the pbar target} &\SI{45}{kHz} &$0.012^\circ$ & \SI{450}{Hz} & \SI{0.105}{\micro\second}& \SI{18.226}{\micro\second}& 0.006\\ \hline	
	\tabincell{c}{RIB B2B transfer from the SIS100 to the CR \\ via the Super FRS}&\SI{2}{kHz}&$0.010^\circ$ &  \SI{108}{Hz} & \SI{0.364}{\micro\second} & \SI{9.779}{\micro\second}& 0.04\\ \hline	
...
\begin{center}
    ...
    \hline
     \tabincell{c}{} & \tabincell{c}{tolerable \\BER}&  \tabincell{c}{tolerable\\ FER}  &  \tabincell{c}{tolerable\\ misordered frame} & \tabincell{c}{upper bound latency \\of WR network}  \\ \hline
       \tabincell{c}{B2B Broadcast} & $2.1\cdot10^{-11}$~\cite{prados_white_2011} &  $1.8\cdot10^{-8}$ &0 &\SI{500}{\us}  \\ \hline
		\tabincell{c}{B2B Unicast} & $4.9\cdot10^{-12}$~\cite{prados_white_2011} & $4.3\cdot10^{-9}$ & 0 & \SI{500}{\us} \\ \hline
    \end{tabular}
\end{center}
\end{table}
    ...
    \hline
     \tabincell{c}{} & \tabincell{c}{FER} & \tabincell{c}{misordered frame} &\tabincell{c}{maximum frame \\transfer latency} \\ \hline
       \tabincell{c}{B2B Broadcast} &  $7.1\cdot10^{-8}$ & 0 & \SI{30}{\us}/switch \\ \hline
		\tabincell{c}{B2B Unicast} 	 & $0$ & 0 & \tabincell{c}{\SI{23}{\us}/4switch}\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
The tolerable layer of the WR switch between the B2B related \gls{SCU}s and between the B2B related SCUs and DM is 
		\begin{equation}
		\begin{aligned}
...
Tab. ~\ref{kicker_SIS100} shows that the sum of the kicker passing time and rise time is shorter than the bunch gap only when the bunch gap is $50\%$ of the cavity rf period, so the SIS100 kicker magnets can be fired instantaneous when bunches are longitudinally compressed. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Compared with the final scenario~\cite{bai_concept_2016}, there are some difference of the test setup.
\begin{itemize}
\item
