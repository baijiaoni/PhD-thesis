The phase shift must be executed slowly enough to preserve the beam emittance, which needs much longer time than the frequency beating method. Besides, many FAIR accelerator pairs are beating automatically due to the non integer ratio of the circumference between two synchrotrons. So there is a preference for FAIR to use the frequency beating method. In this chapter all FAIR use cases with the frequency beating method will be discussed in details. Based on the circumference ratio, there are three scenarios of the B2B transfer for FAIR with the frequency beating method. 
\begin{itemize}
...
\begin{center}
	...
	\tabincell{c}{Circumference\\ ratio} &  $C^l/C^s$ &$\frac{ f_{\mathit{rev}}^{s}}{ f_{\mathit{rev}}^{l}}$& Use cases of FAIR accelerators\\ \hline
  	...
  	\multirow{4}*{\tabincell{c}{$C^l/C^s=m/n+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}=m/n+ \lambda$\\far away from an integer\\ ($|\lambda|<=0.05$)}}&&4.9-0.0004&\tabincell{c}{$H^{+}$ B2B transfer \\from the SIS100 to the CR} \\ \cline{2-4}
&&4.4-0.0046& \tabincell{c}{RIB B2B transfer\\ from the SIS100 to the CR}\\ \cline{2-4}

&2.6-0.003& &\tabincell{c}{B2B transfer \\from the CR to the ESR }\\ \cline{2-4}

&&1.8+0.048&\tabincell{c}{B2B transfer \\from the SIS18 to the ESR \\via FRS} \\ \hline
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of $U^{28+}$ B2B transfer from the SIS18 to the SIS100 with the frequency beating method}
\label{tab:U18to100}
\begin{center}
    \begin{tabular}{ | c | c | c |}
    \hline
   ...
   \tabincell{c}{Frequency \\ of bucket label} & $f_{\mathit{rev}}^{\mathit{SIS100}}$  \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} &  \tabincell{c}{$f_{\mathit{syn}}^{s}+\Delta f=f_{\mathit{rf}}^{\mathit{SIS18}}+\SI{200}{Hz}$=\SI{1.572536}{MHz}+\SI{200}{Hz} \\and\\ $f_{\mathit{syn}}^{l}=f_{\mathit{rf}}^{\mathit{SIS100}}$=\SI{1.572536}{MHz}}\\ \hline
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of $H^{+}$ B2B transfer from the SIS18 to the SIS100 with the frequency beating method}
\label{tab:H18to100}
\begin{center}
    \begin{tabular}{ | c | c | c| }
    \hline
   ...
   \tabincell{c}{Frequency \\ of bucket label} & $f_{\mathit{rev}}^{\mathit{SIS100}}$  \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} &  \tabincell{c}{$f_{\mathit{syn}}^{s}+\Delta f=f_{\mathit{rf}}^{\mathit{SIS18}}+\SI{200}{Hz}$=\SI{1.359358}{MHz}+\SI{200}{Hz} \\and\\ $f_{\mathit{syn}}^{l}=f_{\mathit{rf}}^{\mathit{SIS100}}/2$=\SI{1.359358}{MHz}}\\ \hline
...
In order to inject into the odd and even number buckets, there are two scenarios of the phase difference between the the SIS18 and the SIS100 revolution frequency markers after the synchronization.
\begin{itemize}
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of B2B transfer from the ESR to the CRYRING with the frequency beating method}
...
\end{center}
\end{table}
The $1/2$ the CRYRING revolution frequency marker works for the bucket label. The phase difference between the the ESR and $1/2$ the CRYRING revolution frequency markers equals to $t_{v\_ext}$+$t_{v\_inj}$+$t_{TOF}$ after the synchronization. Detailed parameters of the B2B transfer from the ESR to the CRYRING, please see Appendix \ref{tab:the ESRtothe CRYRING}.
...
the ESR is the target and there exists $\frac{f_{\mathit{rf}}^{s}}{(h^s \cdot\kappa)/Y}=\frac{f_{\mathit{rf}}^{s}}{(2 \cdot2)/4}=f_{\mathit{rev}}^{s}$ ,so substituting $h^X$, $\kappa$, $\lambda$, $f_{\mathit{rf}}^{X}$ and Y into formulas in the second column in Tab.~\ref{Cir ratio close to integer}, the synchronization of h=4 B2B transfer from the SIS18 to the ESR is obtained, see Tab.~\ref{tab:418tothe ESR}. Here we use \SI{30}{MeV/\atomicmassunit} heavy ion as an example. 


\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of h=4 B2B transfer from the SIS18 to the ESR with the frequency beating method}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Use case of h=1 B2B transfer from the SIS18 to the ESR} 
...
Detailed parameters of the $h=1$ B2B transfer from the SIS18 to the ESR, please see Appendix \ref{sec:18tothe ESR}. After the synchronization, the phase difference between the the SIS18 and $1/2$ the ESR cavity rf frequency markers depends on the accumulation method.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Use case of $H^{+}$ B2B transfer from the SIS100 to the CR} 
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of $H^{+}$ B2B transfer from the SIS100 to the CR with the frequency beating method}
...
The GCD of $h^{\mathit{SIS100}}\cdot n=2\cdot5=10$ and $h^{\mathit{CR}} \cdot m=1\cdot 22=22$ is 2, namely $Y=2$. the CR is the small synchrotron and the target and there exists $\frac{Y}{m}=1/22<1$, so substituting $h^X$, $m$, $n$, $\lambda$, $f_{\mathit{rf}}^{X}$ and Y into formulas in the last column in Tab.~\ref{Cir ratio far from integer small}, the synchronization of RIB B2B transfer from the SIS100 to the CR is obtained, see Tab.~\ref{tab:100tothe CRrib}.

\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of RIB B2B transfer from the SIS100 to the CR with the frequency beating method}
...
\end{center}
\end{table}
The the CR is empty before the injection, so the phase jump is preferred for the CR. Detailed parameters of RIB B2B transfer from the SIS100 to the CR, please see Appendix \ref{100tothe CR}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Use case of B2B transfer from the CR to the ESR} 
...
\begin{table}[H]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of B2B transfer from the CR to the ESR with the frequency beating method}
\label{tab:the CRtothe ESR}
\begin{center}
    \begin{tabular}{ | c | c |  }
    \hline
		&  Larger synchrotron (the ESR) is target synchrotron \\ \hline
   		\tabincell{c}{Frequency \\ of bucket label} & $f_{\mathit{syn}}^{l}$  \\ \hline
		\rowcolor[gray]{0.8}
		&\SI{3}{GeV/\atomicmassunit} antiproton	\\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} & $f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{\mathit{CR}}}{13}=\SI{101.290}{\kHz}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{\mathit{ESR}}}{5}=\SI{101.426}{\kHz}$\\ \hline
	...
	\tabincell{c}{Two slightly \\different\\ frequencies} & $f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{\mathit{CR}}}{13}=\SI{86.493}{\kHz}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{\mathit{ESR}}}{5}=\SI{86.608}{\kHz}$\\ \hline
...
After the synchronization, the phase difference between the 1/13 the CR and 1/5 the ESR rf frequency markers depends on the accumulation method. Detailed parameter about the B2B transfer from the CR to the ESR, please see Appendix \ref{sec:the CRtothe ESR}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization of B2B transfer from the SIS18 to the ESR via FRS with the frequency beating method}
...
More parameters about the B2B transfer from the SIS18 to the ESR via FRS, please see Appendix \ref{sec:18tothe ESRvia FRS}. For the detailed realization and implementation of two slightly different frequencies, please see ``Development of the LLRF system for a deterministic Bunch-to-Bucket transfer for FAIR``. 
