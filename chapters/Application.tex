...
\begin{itemize}
	...
	\item The circumference ratio between the large and small synchrotrons is close to an integer.
 	...
 	\item The circumference ratio between the large and small synchrotrons is far away from an integer.
		...
		\end{itemize}
\end{itemize}
Besides, FAIR has many use cases of B2B transfers that the extraction and injection beam have different energy because of targets installed between two synchrotrons (e.g. a Pbar, a FRS and a Super FRS). In this situation, the beam \gls{glos:rev_ratio} between the small and large synchrotrons is equivalent to the circumference ratio between the large and small synchrotrons. For FAIR, there exists the following scenario.
...
\begin{center}
	...
	\tabincell{c}{Circumference\\ ratio} &  $C^l/C^s$ &$f_{\mathit{rev}}^{s}/f_{\mathit{rev}}^{l}$& Use cases of FAIR accelerators\\ \hline
  	...
  	\multirow{4}*{\tabincell{c}{$C^l/C^s=m/n+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}$\\$=m/n+ \lambda$\\far away from \\an integer\\ ($|\lambda|\le 0.05$)}}&\tabincell{c}{not \\applicable}&4.8-0.039&\tabincell{c}{$H^{+}$ B2B transfer \\from the SIS100 to the CR\\ via a Pbar} \\ \cline{2-4}
&\tabincell{c}{not \\applicable}&4.4-0.0046& \tabincell{c}{Rare isotope beams (\gls{RIB})\\ B2B transfer\\from the SIS100 to the CR \\ via a Super FRS}\\ \cline{2-4}

&2.6-0.003& &\tabincell{c}{B2B transfer \\from the CR to the HESR }\\ \cline{2-4}

&\tabincell{c}{not \\applicable}&1.8+0.036&\tabincell{c}{B2B transfer \\from the SIS18 to the ESR \\via a FRS} \\ \hline
...
%   \caption{The frequency of the bucket indication signal for the phase shift method.}{The red dots represent buckets and the blue ones bunches, the black bars chosen by blue rectangle are used by the bucket indication signal. The phase of two rf systems in this example is aligned at $0^\circ$.}
%   \label{bucket_label_phase}
%\end{figure}

The frequency of the bucket indication signal (denoted as ``\gls{symb:bucket_freq}``) depends on the relation between $f_{\mathit{syn}}^{\mathit{trg}}$ and $f_{\mathit{rev}}^{\mathit{trg}}$, see Chap. ~\ref{concept}.

When the large synchrotron is the target, there exists $f_{\mathit{syn}}^{l}=Yf_{\mathit{rev}}^{l}\ge f_{\mathit{rev}}^{l}$. The revolution period is $Y$ times as long as the rf period of $f_{\mathit{syn}}^{l}$. The rf period of $f_{\mathit{syn}}^{l}$ is not long enough to contain all buckets. So $f_\mathit{bucket}=f_{\mathit{rev}}^{l}$ and the length of the synchronization window $T_w=2T_{\mathit{rev}}^{l}$ for this case. Tab.~\ref{Syn_rev_large} shows the formulas for the frequency of the bucket indication signal $f_\mathit{bucket}$, the synchronization frequencies $f_\mathit{syn}^\mathit{X}$, the frequencies of two Reference RF Signals $f_\mathit{B2B}^\mathit{X}$, the frequency of the Synchronization Reference Signal $f_\mathit{syn}^\mathit{REF}$, the length of the synchronization window $T_w$ and the bunch and bucket injection center mismatch $\Delta \phi_\mathit{rf}$ when the large synchrotron is the target. 
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{l}$\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_\mathit{syn}^\mathit{s}=\frac{f_{\mathit{rf}}^{s}}{h^{s}\kappa/Y}$ and $f_\mathit{syn}^\mathit{l}=\frac{f_\mathit{rf}^\mathit{l}}{h^{l}/Y}$ \\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & $f_\mathit{B2B}^\mathit{s}=\frac{f_{\mathit{rev}}^{l}}{f_{\mathit{syn}}^{l}}\cdot (f_{\mathit{syn}}^{s}+\Delta f_\mathit{syn})=\frac{1}{\kappa}(f_{\mathit{rev}}^{s}+\frac{\kappa}{Y}\Delta f_\mathit{syn})$ and $f_\mathit{B2B}^\mathit{l}=f_{\mathit{rev}}^{l}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & $\textit{round} (f_\mathit{rev}^{l}/\SI{100}{kHz})\cdot \SI{100}{kHz}$ \\ \hline
  
	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{l}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm\frac{1}{2}\cdot 2\pi\cdot  |\Delta f_\mathit{syn}|\cdot T_w \cdot \frac{f_{\mathit{rf}}^{l}}{f_{\mathit{syn}}^{l}}$ \\ \hline
    \end{tabular}
\end{center}
\end{table}
...
%   \caption{The frequency of the bucket indication signal for the phase shift method.}{Red dots represent buckets and blue ones bunches, black bars chosen by blue rectangles are used by the bucket indication signal. The phase match of two rf systems in this example is correct phase aligned with $\Delta\phi=0^\circ$. (a) The frequency of the bucket indication signal when $f_{\mathit{syn}}^{s}\ge f_{\mathit{rev}}^{s}$ and the large synchrotron is the target. (b) The frequency of the bucket indication signal when $f_{\mathit{syn}}^{s}<f_{\mathit{rev}}^{s}$.}
%   \label{bucket_label_phase_small}
%\end{figure}
%\begin{figure}[!htb]
%   \centering   
%   \includegraphics*[width=150mm]{bucket_label_frequency_small.jpg}
%   \caption{The frequency of the bucket indication signal for the frequency beating method.}{The red dots represent buckets and the blue ones bunches, the black bars chosen by blue rectangle are used by the bucket indication signal. The phase of two rf systems in this example is correct phase aligned with $\Delta\phi=0^\circ$. (a) The frequency of the bucket indication signal when $f_{\mathit{rev}}^{s}\frac{Y}{\kappa}\ge f_{\mathit{rev}}^{s}$ and the large synchrotron is the target. (b) The frequency of the bucket indication signal when $f_{\mathit{rev}}^{s}\frac{Y}{\kappa}<f_{\mathit{rev}}^{s}$.}
%   \label{bucket_label_frequency_small}
%\end{figure}

When the small synchrotron is the target, the relation between $f_{\mathit{syn}}^{s}=\frac{Y}{\kappa}f_{\mathit{rev}}^{s}$ and $f_{\mathit{rev}}^{s}$ is not fixed. If $f_{\mathit{syn}}^{s}\ge f_{\mathit{rev}}^{s}$, namely $\frac{Y}{\kappa}\ge 1$, the revolution period is $\frac{Y}{\kappa}$ times as long as the rf period of $f_{\mathit{syn}}^{s}$. Hence, $f_\mathit{bucket}=f_{\mathit{rev}}^{s}$ and $T_w=2T_{\mathit{rev}}^{s}$ for this case.
...
Oppositely,  if $f_{\mathit{syn}}^{s}<f_{\mathit{rev}}^{s}$, namely $\frac{Y}{\kappa}<1$, the rf period of $f_{\mathit{syn}}^{s}$ is $\frac{\kappa}{Y}$ times as long as the revolution period. Hence, $f_\mathit{bucket}=f_{\mathit{syn}}^{s}$ and $T_w=2T_{\mathit{syn}}^{s}$. Tab.~\ref{Sync_ratio_int} shows the formulas when the small synchrotron is the target.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{s}$ & $f_{\mathit{syn}}^{s}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & \multicolumn{2}{c|}{$f_\mathit{syn}^\mathit{s}=\frac{f_{\mathit{rf}}^{s}}{h^{s}\kappa/Y}$ and $f_\mathit{syn}^\mathit{l}=\frac{f_\mathit{rf}^\mathit{l}}{h^{l}/Y}$} \\ \hline

	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \tabincell{c}{$f_\mathit{B2B}^\mathit{l}=\frac{f_{\mathit{rev}}^{s}}{f_{\mathit{syn}}^{s}}\cdot (f_{\mathit{syn}}^{l}+\Delta f_\mathit{syn})$\\$=\kappa(f_{\mathit{rev}}^{l}+\frac{1}{Y}\Delta f_\mathit{syn})$ \\and\\ $f_\mathit{B2B}^\mathit{s}=f_{\mathit{rev}}^{s}$} & \tabincell{c}{$f_\mathit{B2B}^\mathit{l}=f_{\mathit{syn}}^{l}+\Delta f_\mathit{syn}$\\$=Y(f_{\mathit{rev}}^{l}+\frac{1}{Y}\Delta f_\mathit{syn})$ \\and \\$f_\mathit{B2B}^\mathit{s}=f_{\mathit{syn}}^{s}=\frac{Y}{\kappa}f_{\mathit{rev}}^{s}$}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} &$\textit{round} (f_\mathit{rev}^{s}/\SI{100}{kHz})\cdot \SI{100}{kHz}$& $\textit{round} (f_\mathit{syn}^{s}/\SI{100}{kHz})\cdot \SI{100}{kHz}$\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{s}$ & $2T_{\mathit{syn}}^{s}$\\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}& \multicolumn{2}{c|}{$\pm\frac{1}{2}\cdot 2\pi \cdot|\Delta f_\mathit{syn}|\cdot T_w \cdot \frac{f_{\mathit{rf}}^{s}}{f_{\mathit{syn}}^{s}}$ }\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
%In fact,  synchronization frequencies could be a fraction (between $1/Y$ and $1$) times of $f_{\mathit{syn}}^{X}$. The bunch-to-bucket injection center mismatch is the mismatch between the cavity rf frequencies within the synchronization window, so it is $f_{\mathit{rf}}^{trg}/f_{\mathit{syn}}^{trg}$ times as large as the phase mismatch between two synchronization frequencies within the synchronization window. When we use one of two synchronization frequencies as the frequency of the bucket indication signal, the synchronization window is two times as long as the rf period of the synchronization frequency of the target synchrotron, so the phase mismatch between two synchronization frequencies within the synchronization window is a constant. The longer the synchronization window, namely the smaller the synchronization frequency, the larger the bunch-to-bucket injection center mismatch. When we use one of two revolution frequencies as the bucket indication signal, the length of the synchronization window is constant. Hence, the bunch-to-bucket injection center mismatch within the synchronization window is also constant. In conclusion, $f_{\mathit{syn}}^{X}$ is chosen as the synchronization frequencies.



%For the frequency beating method, the bucket indication signal indicates the passing time of the first bucket of the target synchrontron, when it is correct phase aligned with the rf system of the source synchrotron, see Fig. ~\ref{bucket_label_frequency_small}. When the large synchrotron is the target, the bucket indication signal is with the frequency of $f_{\mathit{rev}}^{l}$, see Fig. ~\ref{bucket_label_frequency_small} (a). When the small synchrotron is the target, the frequency of the bucket indication signal is depend on the relation between $f_{\mathit{syn}}^{s}=f_{\mathit{rev}}^{s}\frac{Y}{\kappa}$ and $f_{\mathit{rev}}^{s}=\frac{f_{\mathit{rf}}^{s}}{h^s}$. The rule is same as that for the phase shift method. When the large synchrotron is the target, the bucket indication signal is with the frequency of $f_{\mathit{rev}}^{l}$, see Fig. ~\ref{bucket_label_frequency_small} (a) and (b) are for cases $f_{\mathit{rev}}^{s}\frac{Y}{\kappa}\ge f_{\mathit{rev}}^{s}$ and $f_{\mathit{rev}}^{s}\frac{Y}{\kappa}<f_{\mathit{rev}}^{s}$.
%%rev_or_subharmonic1.docx
...
%Here we assume $\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}<f_{\mathit{rev}}^{s}$, if $\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}\ge f_{\mathit{rev}}^{s}$, the bucket indication is with $f_{\mathit{rev}}^{s}$, the $h^s\cdot \kappa)/Y$ in Tab.~\ref{Cir ratio integer} is replaced by 1 and $f_{\mathit{rf}}^{s}$ is replaced by $f_{\mathit{rev}}^{s}$. We assume $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}<f_{\mathit{rev}}^{l}$, if $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}\ge f_{\mathit{rev}}^{l}$, the bucket indication is with $f_{\mathit{rev}}^{l}$ and the $h^l/Y$ in Tab.~\ref{Cir ratio integer} is replaced by 1 and $f_{\mathit{rf}}^{l}$ is replaced by $f_{\mathit{rev}}^{l}$.

%The formulas in Tab.~\ref{Sync_ratio_int} is based on the assumption that $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}<f_{\mathit{rev}}^{l}$ (the large synchrotron is the target), namly the frequency for beating is smaller than the revolution frequency, so the period of the frequency for beating is long enough to indicate all buckets in one revolution period. If $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}\ge f_{\mathit{rev}}^{l}$, the period of the frequency for beating is shorter than the revolution period. Hence, the revolution period is used to indicated all buckets. So does for the small synchrotron. The formulas in Tab.~\ref{Syn_rev_ratio_int} is with the assumption of $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}\ge f_{\mathit{rev}}^{l}$.
...
%	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \multicolumn{2}{c|}{$\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}+\Delta f$ or $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}+\Delta f$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$}\\ \hline
%	\tabincell{c}{$T_w$}& $2\cdot T_{\mathit{rev}}^{l}$ & $2\cdot T_{\mathit{rev}}^{s}$\\ \hline
%	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm\frac{1}{2}\cdot\frac{2\cdot T_{\mathit{rev}}^{l}}{1/\Delta f}\cdot360^\circ$ & $\pm\frac{1}{2}\cdot\frac{2\cdot T_{\mathit{rev}}^{s}}{1/\Delta f}\cdot360^\circ$\\ \hline
%    \end{tabular}
%\end{center}
%\end{table}
...
%
%There exists $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}\ge f_{\mathit{rev}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l}$, so the formulas in Tab.~\ref{Syn_rev_ratio_int} is applicable.
%%\begin{table}[!htb]
%%\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
...
Because the SIS100 is the large synchrotron and the target, substituting $h^X$, $\kappa$, $f_{\mathit{rf}}^{X}$, $f_{\mathit{rev}}^{X}$ and $Y$ into formulas in Tab.~\ref{Syn_rev_large}, the parameters related to the $U^{28+}$ B2B transfer from the SIS18 to the SIS100 is obtained, see Tab.~\ref{tab:U18to100}. Here we assume that the SIS18 is detuned with \SI{200}{Hz} for the synchronization frequency $f_{\mathit{syn}}^{\mathit{SIS18}}$. 
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{\mathit{SIS100}}=\SI{0.157254}{MHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} &  \tabincell{c}{$f_{\mathit{syn}}^{SIS18}=f_{\mathit{rf}}^{\mathit{SIS18}}=$\SI{1.572536}{MHz} \\and\\ $f_{\mathit{syn}}^{SIS100}=f_{\mathit{rf}}^{\mathit{SIS100}}$=\SI{1.572536}{MHz}}\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} &  \tabincell{c}{$f_\mathit{B2B}^\mathit{SIS18}=\frac{1}{5}(f_{\mathit{rev}}^{\mathit{SIS18}}+\frac{1}{2}\Delta f_\mathit{syn})=$\SI{0.1572536}{MHz}+\SI{20}{Hz} \\and\\ $f_\mathit{B2B}^\mathit{SIS100}$=\SI{0.1572536}{MHz}}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{200}{kHz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{\mathit{SIS100}}=\SI{12.718}{us}$\\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm0.5^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}

The target time difference between the SIS18 and SIS100 rf systems equals to $t_{v\_ext}+t_{v\_inj}+t_{TOF}$, provided by the SM. The SIS100 revolution frequency works for the bucket indication. When the 1st and 2nd buckets are to be filled, $t_{pattern}=0$. When the 3rd and 4th buckets are to be filled, $t_{pattern}=T_{\mathit{rev}}^{\mathit{SIS18}}$. When the 5th and 6th buckets are to be filled, $t_{pattern}= 2 \cdot T_{\mathit{rev}}^{\mathit{SIS18}}$. When the 7th and 8th buckets are to be filled, $t_{pattern}= 3 \cdot T_{\mathit{rev}}^{\mathit{SIS18}}$. Detailed parameters of the $U^{28+}$ B2B transfer from the SIS18 to the SIS100, please see Appendix \ref{sec:18to100}.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{\mathit{SIS100}}=\SI{0.271872}{MHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} &   \tabincell{c}{$f_{\mathit{syn}}^{SIS18}=f_{\mathit{rf}}^{\mathit{SIS18}}$=\SI{1.359358}{MHz} \\and\\ $f_{\mathit{syn}}^{SIS100}=f_{\mathit{rf}}^{\mathit{SIS100}}/2$=\SI{1.359358}{MHz}}\\ \hline

	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} &  \tabincell{c}{$f_\mathit{B2B}^\mathit{SIS18}=\frac{1}{5}(f_{\mathit{rev}}^{SIS18}+\Delta f_\mathit{syn})=$\SI{0.271872}{MHz}+\SI{40}{Hz} \\and\\ $f_{\mathit{B2B}}^{SIS100}$=\SI{0.271872}{MHz}}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{300}{kHz}\\ \hline

	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{\mathit{SIS100}}=\SI{7.356}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm0.6^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
%%%%%%%%%%%
...
The SIS100 revolution frequency works for the bucket indication. In order to inject into the odd and even number buckets, there are two scenarios of the target time difference between the SIS18 and SIS100 rf systems.
\begin{itemize}
	\item Injection into odd number buckets
		
		The target time difference between the SIS18 and SIS100 rf systems equals to $t_{v\_ext}+t_{v\_inj}+t_{TOF}$. When the 1st bucket is to be filled, $t_{pattern}$=0. When the 3rd bucket is to be filled, $t_{pattern}=1 \cdot T_{\mathit{rev}}^{\mathit{SIS18}}$. 
	\item Injection into even number buckets
	
		The target time difference between the SIS18 and SIS100 rf systems equals to $t_{v\_ext}+t_{v\_inj}+t_{TOF}-T_{\mathit{rf}}^{\mathit{SIS100}}$. When the 2nd bucket is to be filled, $t_{pattern}=0$. When the 4th bucket is to be filled, $t_{pattern}=1\cdot T_{\mathit{rev}}^{\mathit{SIS18}}$. 
...
%The slightly different frequencies are $\frac{f_{\mathit{rf}}^{s}}{(h^s \cdot \kappa)/Y}$ and $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ and the beating frequency is the difference between $\frac{f_{\mathit{rf}}^{s}}{(h^s \cdot \kappa)/Y}$ and $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$. If the small synchrotron is the target, $\frac{f_{\mathit{rf}}^{s}}{(h^s \cdot \kappa)/Y}$ works as for the bucket indication, or $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ works for the bucket indication. 
...
\begin{center}
	...
	&  Large synchrotron is target synchrotron \\ \hline
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{l}$\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ \\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & $f_\mathit{B2B}^\mathit{s}=\frac{f_{\mathit{rev}}^{l}}{f_{\mathit{syn}}^{l}}\cdot f_{\mathit{syn}}^{s}=\frac{1}{\kappa}f_{\mathit{rev}}^{s}$ and $f_\mathit{B2B}^\mathit{l}=f_{\mathit{rev}}^{l}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & $\textit{round} (f_\mathit{rev}^{l}/\SI{100}{kHz})\cdot \SI{100}{kHz}$ \\ \hline
	\tabincell{c}{$\Delta f$} & $|f_{\mathit{syn}}^{s}-f_{\mathit{syn}}^{l}|$ \\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{l}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm\frac{1}{2}\cdot 2\pi \cdot |\Delta f|\cdot T_w \cdot\frac{f_{\mathit{rf}}^{l}}{f_{\mathit{syn}}^{l}}$ \\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{s}$ &$f_{\mathit{syn}}^{s}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & \multicolumn{2}{c|}{$f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$} \\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \tabincell{c}{$f_\mathit{B2B}^\mathit{l}=\frac{f_{\mathit{rev}}^{s}}{f_{\mathit{syn}}^{s}}\cdot f_{\mathit{syn}}^{l}=\kappa f_{\mathit{rev}}^{l}$ \\and\\ $f_\mathit{B2B}^\mathit{s}=f_{\mathit{rev}}^{s}$} & \tabincell{c}{$f_\mathit{B2B}^\mathit{l}=f_{\mathit{syn}}^{l}=Y f_{\mathit{rev}}^{l}$ \\and \\$f_\mathit{B2B}^\mathit{s}=f_{\mathit{syn}}^{s}=\frac{Y}{\kappa} f_{\mathit{rev}}^{s}$}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & $\textit{round} (f_\mathit{rev}^{s}/\SI{100}{kHz})\cdot \SI{100}{kHz}$ &$\textit{round} (f_\mathit{syn}^{s}/\SI{100}{kHz})\cdot \SI{100}{kHz}$ \\ \hline
	\tabincell{c}{$\Delta f$} & \multicolumn{2}{c|}{$|f_{\mathit{syn}}^{s}-f_{\mathit{syn}}^{l}|$}\\ \hline

	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{s}$ & $2T_{\mathit{syn}}^{s}$\\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}& \multicolumn{2}{c|}{$\pm\frac{1}{2}\cdot 2\pi \cdot |\Delta f|\cdot T_w \cdot\frac{f_{\mathit{rf}}^{s}}{f_{\mathit{syn}}^{s}}$}\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{\mathit{ESR}}=\SI{0.685651}{\MHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{SIS18}=f_{\mathit{rf}}^{\mathit{SIS18}}=\SI{1.373201}{\MHz}$ and $f_{\mathit{syn}}^{ESR}=f_{\mathit{rf}}^{\mathit{ESR}}=\SI{1.371302}{\MHz}$\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & $f_{\mathit{B2B}}^{SIS18}=2f_{\mathit{rev}}^{\mathit{SIS18}}= \SI{0.686601}{\MHz}$ and $f_{\mathit{B2B}}^{ESR}=f_{\mathit{rev}}^{\mathit{ESR}}=\SI{0.685651}{\MHz}$\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{700}{kHz} \\ \hline
	\tabincell{c}{$\Delta f$} & \SI{1899}{\Hz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{\mathit{ESR}}=\SI{2.917}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm1.0^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{syn}}^{ESR}=\SI{988.388}{\kHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{SIS18}=f_{\mathit{rf}}^{\mathit{SIS18}}=\SI{989.756}{\kHz}$ and $f_{\mathit{syn}}^{ESR}=f_{\mathit{rf}}^{\mathit{ESR}}/2=\SI{988.388}{\kHz}$\\ \hline
	...
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{\mathit{ESR}}=\SI{2.034}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm1.0^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}

Detailed parameters of the $h=1$ B2B transfer from the SIS18 to the ESR, please see Appendix \ref{sec:18toESR}. The target time difference between the SIS18 and ESR rf systems depends on the accumulation method.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{syn}}^{CRYRING}=\SI{0.686600}{MHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & \tabincell{c}{$f_{\mathit{syn}}^{ESR}=f_{\mathit{rf}}^{\mathit{ESR}}=\SI{0.685651}{MHz}$ \\and \\$f_{\mathit{syn}}^{CRYRING}=f_{\mathit{rf}}^{\mathit{CRYRING}}/2$=\SI{0.686600}{MHz}}\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \tabincell{c}{$f_{\mathit{B2B}}^{ESR}=f_{\mathit{rev}}^{\mathit{ESR}}=\SI{0.685651}{MHz}$ \\and \\$f_{\mathit{B2B}}^{CRYRING}=f_{\mathit{rev}}^{\mathit{CRYRING}}/2=\SI{0.686600}{MHz}$}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{700}{kHz}\\ \hline
	\tabincell{c}{$\Delta f$} & \SI{949}{\Hz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{\mathit{CRYRING}}=\SI{2.912}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm1.0^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
The CRYRING synchronization frequency works for the bucket indication. The target time difference between the ESR and CRYRING rf systems equals to $t_{v\_ext}+t_{v\_inj}+t_{TOF}$. Detailed parameters of the B2B transfer from the ESR to the CRYRING, please see Appendix \ref{tab:ESRtoCRYRING}.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{l}$ &$f_{\mathit{syn}}^{l}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & \multicolumn{2}{c|}{$f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot m)/Y}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{(h^l\cdot n)/Y}$}\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \tabincell{c}{$f_{\mathit{B2B}}^{s}=\frac{f_{\mathit{rev}}^{l}}{f_{\mathit{syn}}^{l}}\cdot f_{\mathit{syn}}^{s}=\frac{n}{m}f_{\mathit{rev}}^{s} $ \\and\\ $f_{\mathit{B2B}}^{l}=f_{\mathit{rev}}^{l}$} &\tabincell{c}{$f_{\mathit{B2B}}^{s}=f_{\mathit{syn}}^{s}=\frac{Y}{m}f_{\mathit{rev}}^{s} $\\ and\\ $f_{\mathit{B2B}}^{l}=f_{\mathit{syn}}^{l}=\frac{Y}{n}f_{\mathit{rev}}^{l}$}\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & $\textit{round} (f_\mathit{rev}^{l}/\SI{100}{kHz})\cdot \SI{100}{kHz}$&$\textit{round} (f_\mathit{syn}^{l}/\SI{100}{kHz})\cdot \SI{100}{kHz}$ \\ \hline
	\tabincell{c}{$\Delta f$} & \multicolumn{2}{c|}{$|f_{\mathit{syn}}^{l}-f_{\mathit{syn}}^{s}|$}\\ \hline

	\tabincell{c}{$T_w$}& $2T_{\mathit{rev}}^{l}$ & $2T_{\mathit{syn}}^{l}$\\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&\multicolumn{2}{c|}{$\pm\frac{1}{2}\cdot 2\pi \cdot |\Delta f|\cdot T_w \cdot\frac{f_{\mathit{rf}}^{l}}{f_{\mathit{syn}}^{l}}$}\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{rev}}^{s}$ &$f_{\mathit{syn}}^{s}$ \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & \multicolumn{2}{c|}{$f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot m)/Y}$ and $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{(h^l\cdot n)/Y}$}\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & \tabincell{c}{$f_{\mathit{B2B}}^{s}=f_{\mathit{rev}}^{s} $ \\and\\ $f_{\mathit{B2B}}^{l}=\frac{f_{\mathit{rev}}^{s}}{f_{\mathit{syn}}^{s}}\cdot f_{\mathit{syn}}^{l}=\frac{m}{n}f_{\mathit{rev}}^{l}$} &\tabincell{c}{$f_{\mathit{B2B}}^{s}=f_{\mathit{syn}}^{s}=\frac{Y}{m}f_{\mathit{rev}}^{s} $\\ and\\ $f_{\mathit{B2B}}^{l}=f_{\mathit{syn}}^{l}=\frac{Y}{n}f_{\mathit{rev}}^{l}$}\\ \hline
...
The CR is the small synchrotron and the target and there exists $\frac{Y}{m}=1/24<1$, so substituting $h^X$, $m$, $n$, $\lambda$, $f_{\mathit{rf}}^{X}$ and Y into formulas of the case (2) in Tab.~\ref{Cir ratio far from integer small}, the parameters related to the $H^{+}$ B2B transfer from the SIS100 to the CR is obtained, see Tab.~\ref{tab:100tothe CRproton}.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{syn}}^{CR}=\SI{54.866}{\kHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{SIS100}=f_{\mathit{rf}}^{\mathit{SIS100}}/25=\SI{55.316}{\kHz}$ and $f_{\mathit{syn}}^{CR}=f_{\mathit{rf}}^{\mathit{CR}}/24=\SI{54.866}{\kHz}$\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & $f_{\mathit{B2B}}^{SIS100}=f_{\mathit{rev}}^{\mathit{SIS100}}/5=\SI{55.316}{\kHz}$ and $f_{\mathit{B2B}}^{CR}=f_{\mathit{rev}}^{\mathit{CR}}/24=\SI{54.866}{\kHz}$\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{100}{kHz} \\ \hline
	\tabincell{c}{$\Delta f$} & \SI{450}{\Hz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{CR}=\SI{36.452}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm70.9^\circ$\\ \hline

    \end{tabular}
\end{center}
\end{table}
There exists an inherent big bunch-to-bucket injection center mismatch. If this mismatch is not acceptable, the phase jump of the CR is an alternate solution, because the CR is empty before the injection. The difficulty is how to precisely synchronize the phase jump with the beam transfer, which is beyond the scope of this dissertation. Detailed parameters of the $H^{+}$ B2B transfer from the SIS100 to the CR , please see Appendix \ref{100toCR}.
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{syn}}^{CR}=\SI{102.218}{\kHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{SIS100}=f_{\mathit{rf}}^{\mathit{SIS100}}/5=\SI{102.326}{\kHz}$ and $f_{\mathit{syn}}^{CR}=f_{\mathit{rf}}^{\mathit{CR}}/11=\SI{102.218}{\kHz}$\\ \hline
	...
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{CR}=\SI{19.558}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm4.2^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\label{B2B_CR_HESR}
One bunch of the CR is injected into one bucket of the HESR. The beam is accumulated in the HESR ~\cite{toelle_hesr_2007}. The large synchrotron is the HESR and the small one is the CR. $h^{\mathit{HESR}}=1$ and $h^{\mathit{CR}}=1$. The circumference ratio between the HESR and the CR is
...
\begin{center}
	...
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{CR}=\frac{f_{\mathit{rf}}^{\mathit{CR}}}{13}=\SI{101.290}{\kHz}$ and $f_{\mathit{syn}}^{HESR}=\frac{f_{\mathit{rf}}^{\mathit{HESR}}}{5}=\SI{101.426}{\kHz}$\\ \hline
	\tabincell{c}{$f_\mathit{B2B}^\mathit{X}$} & $f_{\mathit{B2B}}^{CR}=\frac{f_{\mathit{rev}}^{\mathit{CR}}}{13}=\SI{101.290}{\kHz}$ and $f_{\mathit{B2B}}^{HESR}=\frac{f_{\mathit{rev}}^{\mathit{HESR}}}{5}=\SI{101.426}{\kHz}$\\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{100}{kHz} \\ \hline
	\tabincell{c}{$\Delta f$} & \SI{136}{\Hz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{HESR}=\SI{19.719}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm2.4^\circ$\\ \hline
		\rowcolor[gray]{0.8}
 		&\SI{740}{MeV/\atomicmassunit} RIB \\ \hline
   		$f_\mathit{bucket}$ & $f_{\mathit{syn}}^{HESR}=\SI{86.608}{\kHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{CR}=f_{\mathit{rf}}^{\mathit{CR}}/13=\SI{86.493}{\kHz}$ and $f_{\mathit{syn}}^{HESR}=f_{\mathit{rf}}^{\mathit{HESR}}/5=\SI{86.608}{\kHz}$\\ \hline
	...
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{HESR}=\SI{23.090}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm2.4^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}

The target time difference between two rf systems depends on the accumulation method. Detailed parameter about the B2B transfer from the CR to the HESR, please see Appendix \ref{sec:CRtoHESR}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Parameters related to the B2B transfer from the SIS18 to the ESR via a FRS with the frequency beating method}
...
\begin{center}
   ...
   $f_\mathit{bucket}$ & $f_{\mathit{syn}}^{ESR}=\SI{219.642}{\kHz}$  \\ \hline
	\tabincell{c}{$f_\mathit{syn}^\mathit{X}$} & $f_{\mathit{syn}}^{SIS18}=f_{\mathit{rf}}^{\mathit{SIS18}}/5=\SI{215.393}{\kHz}$ and $f_{\mathit{syn}}^{ESR}=f_{\mathit{rf}}^{\mathit{ESR}}/9=\SI{219.642}{\kHz}$\\ \hline
	...
	\tabincell{c}{$f_\mathit{syn}^\mathit{REF}$} & \SI{200}{kHz} \\ \hline
	\tabincell{c}{$\Delta f$} & \SI{4.249}{\kHz}\\ \hline
	\tabincell{c}{$T_w$}& $2T_{\mathit{syn}}^{ESR}=\SI{9.106}{us}$ \\ \hline
	\tabincell{c}{$\Delta \phi_\mathit{rf}$}&$\pm62.28^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}

There exists an inherent big bunch-to-bucket injection center mismatch. If this mismatch is not acceptable, the phase jump of the ESR is an alternate solution, because the ESR is empty before the injection. The difficulty is how to precisely synchronize the phase jump with the beam transfer, which is beyond the scope of this dissertation. More parameters about the B2B transfer from the SIS18 to the ESR via a FRS, please see Appendix \ref{sec:18toESRvia FRS}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Summary of the formulas related to the B2B transfer}
In this section, all the formulas are summarized.  Tab.~\ref{B2B_transfer_rule} summarizes the formulas related to the B2B transfer when the large synchrotron is the target. Tab.~\ref{B2B_transfer_rule1} summarizes the formulas when the small synchrotron is the target and the revolution period is longer than the rf period of the synchronization frequency of the target synchrotron. Tab.~\ref{B2B_transfer_rule11} summarizes the formulas when the small synchrotron is the target and the revolution period is shorter than the rf period of the synchronization frequency of the target synchrotron.
\begin{landscape} 
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
...
\begin{center}
    \begin{tabular}{| c | c | c | c | c | c | c | c|}
    \hline
	\tabincell{c}{Circumference\\ratio} &  \tabincell{c}{Cavity rf\\frequency ratio \\$f_{\mathit{rf}}^{l}/f_{\mathit{rf}}^{s}$}& \tabincell{c}{Bucket indication \\ signal\\$f_\mathit{bucket}$}&\tabincell{c}{Synchronization \\frequencies\\$f_{\mathit{syn}}^{X}$}& \tabincell{c}{Reference RF Signal\\ $f_{\mathit{B2B}}^{X}$} \\ \hline
 \tabincell{c}{$C^l/C^s=\kappa$ \\Integer} &  \tabincell{c}{$\frac{h^l}{h^s\cdot \kappa}$\\ \\ \tabincell{c}{Y=GCD$(h^l,h^s \kappa)$}} & \tabincell{c}{$f_{\mathit{rev}}^{l}$} & \tabincell{c}{$f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$\\ and\\ $f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$\\ \\ $\Delta f=\Delta f_\mathit{syn}$} &\tabincell{c}{$f_\mathit{B2B}^\mathit{s}=\frac{f_{\mathit{rev}}^{l}}{f_{\mathit{syn}}^{l}}\cdot (f_{\mathit{syn}}^{s}+\Delta f_\mathit{syn})$ \\and \\$f_\mathit{B2B}^\mathit{l}=f_{\mathit{rev}}^{l}$} \\ \hline
...
\multicolumn{5}{|l|}{\textit{Note:} $f_\mathit{syn}^\mathit{REF}=\textit{round} (f_\mathit{bucket}/\SI{100}{kHz})\cdot \SI{100}{kHz}$, $T_w=2/f_{\mathit{bucket}}$ and $\Delta \phi_\mathit{rf}=\pm\frac{1}{2}\cdot 2\pi \cdot |\Delta f|\cdot T_w \cdot\frac{f_{\mathit{rf}}^{l}}{f_{\mathit{syn}}^{l}}$} \\ \hline
    \end{tabular}
\end{center}
\end{table}
\end{landscape} 
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Summary of the formulas related to the B2B transfer when the small synchrotron is the target and the revolution period is longer than the rf period of the synchronization frequency of the target synchrotron}{}
...
&  \tabincell{c}{$\frac{h^l}{h^s\cdot \kappa}$\\ \\ $Y=GCD(h^l,h^s \kappa)$} 
& \tabincell{c}{$Y/\kappa\ge1, f_{\mathit{rev}}^{s}$} & \tabincell{c}{$f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ \\and \\$f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$\\ \\ $\Delta f=\Delta f_\mathit{syn}$}
&\tabincell{c}{$f_\mathit{B2B}^\mathit{l}=\frac{f_{\mathit{rev}}^{s}}{f_{\mathit{syn}}^{s}}\cdot (f_{\mathit{syn}}^{l}+\Delta f_\mathit{syn})$ \\and\\ $f_\mathit{B2B}^\mathit{s}=f_{\mathit{rev}}^{s}$}\\ \hline
...
\multicolumn{5}{|l|}{\textit{Note:} $f_\mathit{syn}^\mathit{REF}=\textit{round} (f_\mathit{bucket}/\SI{100}{kHz})\cdot \SI{100}{kHz}$, $T_w=2/f_{\mathit{bucket}}$ and $\Delta \phi_\mathit{rf}=\pm\frac{1}{2}\cdot 2\pi \cdot |\Delta f|\cdot T_w \cdot\frac{f_{\mathit{rf}}^{s}}{f_{\mathit{syn}}^{s}}$} \\ \hline
    \end{tabular}
\end{center}
\end{table}
\end{landscape} 
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Summary of the formulas related to the B2B transfer when the small synchrotron is the target and the revolution period is shorter than the rf period of the synchronization frequency of the target synchrotron}{}
...
\begin{center}
 ...
 \tabincell{c}{$C^l/C^s=\kappa$ \\Integer} &  \tabincell{c}{$\frac{h^l}{h^s\cdot \kappa}$\\ \\ $Y=GCD(h^l,h^s \kappa)$} 
& \tabincell{c}{$Y/\kappa<1, f_{\mathit{syn}}^{s}$} 
& \tabincell{c}{$f_{\mathit{syn}}^{l}=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ \\and\\ $f_{\mathit{syn}}^{s}=\frac{f_{\mathit{rf}}^{s}}{(h^s\cdot \kappa)/Y}$ \\ \\ $\Delta f=\Delta f_\mathit{syn}$}
&\tabincell{c}{$f_\mathit{B2B}^\mathit{l}=f_{\mathit{syn}}^{l}+\Delta f_\mathit{syn}$ \\ and \\$f_\mathit{B2B}^\mathit{s}=f_{\mathit{syn}}^{s}$}\\ \hline
