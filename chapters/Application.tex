...
\begin{center}
	...
	\tabincell{c}{Circumference\\ ratio} &  $C^l/C^s$ &$\frac{ f_{\mathit{rev}}^{s}}{ f_{\mathit{rev}}^{l}}$& Use cases of FAIR accelerators\\ \hline
  	...
  	\multirow{4}*{\tabincell{c}{$C^l/C^s=m/n+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}=m/n+ \lambda$\\far away from an integer\\ ($|\lambda|<=0.05$)}}&not applicable&4.9-0.0004&\tabincell{c}{$H^{+}$ B2B transfer \\from the SIS100 to the CR} \\ \cline{2-4}
...
&not applicable&1.8+0.048&\tabincell{c}{B2B transfer \\from the SIS18 to the ESR \\via FRS} \\ \hline
...
Two synchronization rf frequencies are beating automatically. When the large synchrotron is the target, the frequency of the bucket label signal depends on the relation between $f_{\mathit{syn}}^{s}$ and $f_{\mathit{rev}}^{s}$. When $f_{\mathit{syn}}^{l}>=f_{\mathit{rev}}^{l}$, namely $\frac{Y}{n}>=1$, the revolution period is longer than the period for the phase alignment. Hence, the frequency of the bucket label signal is $f_{\mathit{rev}}^{s}$. Or the bucket label signal with the frequency of $f_{\mathit{syn}}^{l}$. Tab.~\ref{Cir ratio far from integer large} shows the formulas when the large synchrotron is the target.
...
\end{center}
\end{table}
When the small synchrotron is the target, the frequency of the bucket label signal depends on the relation between $f_{\mathit{syn}}^{s}$ and $f_{\mathit{rev}}^{s}$. When $f_{\mathit{syn}}^{s}>=f_{\mathit{rev}}^{s}$, namely $\frac{Y}{m}>=1$ the revolution period is longer than the period for the phase alignment. Hence, the frequency of the bucket label signal is $f_{\mathit{rev}}^{s}$. Or the bucket label signal with the frequency of $f_{\mathit{syn}}^{s}$. Tab.~\ref{Cir ratio far from integer small} shows the formulas when the small synchrotron is the target.
