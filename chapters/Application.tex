...
\begin{center}
    ...
    \hline
	&\tabincell{c}{Circumference\\ratio} &  \tabincell{c}{RF cavity\\frequency ratio \\$f_{rf}^{l}/f_{rf}^{s}$\tablefootnote{$\frac{f_rf^{l}}{f_{rf}{s}}=\frac{h^l f_rev^{l}}{h^s  f_{rev}^{s}}=\frac{h^l C^{s}}{h^s C_l}$}}& \tabincell{c}{Bucket label\\(large or small is \\target synchrotron)}&\tabincell{c}{Frequency beating\\Two slightly \\different frequencies}& \tabincell{c}{Frqeuency beating\\Mismatch\tablefootnote{Bunch and bucket center mismatch}\\(large or small is \\target synchrotron)} \\ \hline
    Type A & \tabincell{c}{$C^l/C^s=\kappa$ \\Integer} &  \tabincell{c}{$\frac{h^l}{h^s\times \kappa}$\\ \\ $Y=GCD(h^l,h^s\times \kappa)$\tablefootnote{Greatest Common Divider}} & $\frac{f_{rf}^{l}}{h^l/Y} or \frac{f_{rf}^{s}}{(h^s\times \kappa)/Y}$ & \tabincell{c}{$\frac{f_{rf}^{l}}{h^l/Y} and \frac{f_{rf}^{s}}{(h^s\times \kappa)/Y}+\Delta f$\\  or\\ $\frac{f_{rf}^{l}}{h^l/Y}+\Delta f and \frac{f_{rf}^{s}}{(h^s\times \kappa)/Y}$}&\tabincell{c}{$\pm\frac{1}{2}\times\frac{2\times (h^l/Y) \times T_{rev}^{l}}{1/\Delta f}\times360^\circ$ \\ or\\ $\pm\frac{1}{2}\times\frac{2\times ([(h^s\times \kappa)/Y])\times T_{rev}^{s}}{1/\Delta f}\times360^\circ$}\\ \hline
											
     Type B & 	\tabincell{c}{$C^l/C^s=\iota+ \lambda$ \\ or \\ $f{rev}^{s}/f{rev}^{l}\tablefootnote{The extraction and injection beam have different energy because of the target, e.g. FRS, Pbar}=\iota+ \lambda$\\close to integer\\($\iota$ is integer) }&\tabincell{c}{$\frac{h^l}{h^s\times (\iota+ \lambda)}$\\ \\ $Y=GCD(h^l,h^s\times \iota)$} & $\frac{f_{rf}^{l}}{h^l/Y} or \frac{f_{rf}^{s}}{(h^s\times \iota)/Y}$ & $\frac{f_{rf}^{l}}{h^l/Y} and \frac{f_{rf}^{s}}{(h^s\times \iota)/Y}$ & \tabincell{c}{$\pm\frac{1}{2}\times \frac{2\times (h^l/Y) \times T_{rev}^{l}}{1/\Delta f}\times360^\circ$ \\ or \\ $\pm\frac{1}{2}\times\frac{2\times [(h^s\times \iota)/Y]\times T_{rev}^{s}}{1/\Delta f}\times360^\circ$}  \\ \hline


 Type C &\tabincell{c}{$C^l/C^s=\iota+ \lambda$ \\ or \\ $f{rev}^{s}/f{rev}^{l}=\iota+ \lambda$\\far away from integer\\($\iota$ is expressed by $\frac{m}{n}$)}&\tabincell{c}{ $\frac{h^l}{h^s \times (m/n+ \lambda)}\tablefootnote{$\frac{f_rf^{l}}{f_{rf}{s}}=\frac{h^l f_rev^{l}}{h^s  f_{rev}^{s}}=\frac{h^l C^{s}}{h^s C_l}=\frac{h^l}{h^s (m/n+\lambda)}=\frac{h^l\times n}{h^s \times m+ h^s \times\lambda\times n}$}$\\ \\ \tabincell{c}{Y=GCD\\$(h^l\times n,h^s \times m)$}}&$\frac{f_{rf}^{l}}{(h^l\times n)/Y} or \frac{f_{rf}^{s}}{(h^s\times m)/Y}$ & $\frac{f_{rf}^{l}}{(h^l\times n)/Y} and \frac{f_{rf}^{s}}{(h^s\times m)/Y}$ & \tabincell{c}{$\pm\frac{1}{2}\times\frac{2\times[(h^l\times n)/Y] \times T_{rev}^{l}}{1/\Delta f}\times360^\circ$ \\ or \\ $\pm\frac{1}{2}\times\frac{2\times [(h^s\times m)/Y]\times T_{rev}^{s}}{1/\Delta f}\times360^\circ$}  \\ \hline
...
\begin{center}
    \begin{tabular}{ | c | c | c | c | c | c | c | c |}
    \hline
	\tabincell{c}{Type} &  $C^l/C^s$ &$\frac{ f_{rev}^{l}}{ f_{rev}^{s}}$&$h^l$ & $h^s$ &  $f_{rf}^{l}/f_{rf}^{s}$\tablefootnote{$\frac{f_rf^{l}}{f_{rf}{s}}=\frac{h^l f_rev^{l}}{h^s  f_{rev}^{s}}=\frac{h^l C^{s}}{h^s C_l}$} & User case of FAIR accelerators\\ \hline
...
& 5&& 10 & 1 & $\frac{h^l}{h^s\times \kappa}=\frac{10}{1\times 5}=2$ &\tabincell{c}{$H^{+}$ B2B transfer \\from SIS18 to SIS100} \\ \cline{2-7}
     	...
     	\multirow{3}*{B}&2-0.003& &4&1 & $\frac{h^l}{h^s\times (\iota+ \lambda)}=\frac{4}{1\times (2-0.003)}$ & \tabincell{c}{h=4 B2B transfer \\from SIS18 to ESR} \\ \cline{2-7}
 								  	  &2-0.003&& 1&1 &$\frac{h^l}{h^s\times (\iota+ \lambda)}=\frac{1}{1\times (2-0.003)}$ & \tabincell{c}{h=1 B2B transfer\\ from SIS18 to ESR}\\ \cline{2-7}
 									&&5-0.104& 5&1 &$\frac{f_{rf}^{Ext}}{f_{rf}^{Inj}}=\iota+ \lambda=1+0.021$ & \tabincell{c}{$H^{+}$ B2B transfer\\ from SIS100 to CR -----} \\ \hline

  	\multirow{3}*{C}& &5-0.104& 2&1 & $\frac{f_{rf}^{Inj}}{f_{rf}^{Ext}}=\frac{m}{n}+ \lambda=\frac{5}{2}-0.054$ & \tabincell{c}{RIB B2B transfer \\from SIS100 to CR----} \\ \cline{2-7}
...
& 2.6-0.003& &1&1 &$\frac{h^l}{h^s \times (m/n+ \lambda)}\tablefootnote{$\frac{f_rf^{l}}{f_{rf}{s}}=\frac{h^l f_rev^{l}}{h^s  f_{rev}^{s}}=\frac{h^l C^{s}}{h^s C_l}=\frac{h^l}{h^s (m/n+\lambda)}=\frac{h^l\times n}{h^s \times m+ h^s \times\lambda\times n}$}=\frac{1}{1 \times (13/5-0.003)}$ &\tabincell{c}{B2B transfer\\ from CR to HESR} \\ \cline{2-7}
