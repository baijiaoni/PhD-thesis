...
\begin{center}
    ...
    \hline
	\tabincell{c}{Circumference\\ ratio \footnote{$\kappa$, $m$ and $n$ are integer and $\lambda$ is decimal number, smaller than $10^{-2}$}} &  $C^l/C^s$ &$\frac{ f_{\mathit{rev}}^{s}}{ f_{\mathit{rev}}^{l}}$& Use case of FAIR accelerators\\ \hline
  	...
  	\multirow{4}*{\tabincell{c}{$C^l/C^s=m/n+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}=m/n+ \lambda$\\far away from an integer)}}&&4.9-0.0004&\tabincell{c}{$H^{+}$ B2B transfer \\from SIS100 to CR} \\ \cline{2-4}
&&4.4-0.0046& \tabincell{c}{RIB B2B transfer\\ from SIS100 to CR}\\ \cline{2-4}
...
%For the phase shift method, the bucket label signal indicates neither all of the time ($f_{\mathit{rev}}^{l}$ or $f_{\mathit{rev}}^{s}$), when the first bucket of the target synchrotron passes by the rf virtual cavity, nor all of the time ($\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ or $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$), when the phase of two rf systems are aligned. It indicates only the passing time of the first bucket, when it is phase aligned with the rf system of the source synchrotron, see Fig. ~\ref{bucket_label_phase}.
For the phase shift method, the bucket label signal indicates the passing time of the first bucket of the target synchrotron, when it is phase aligned with the rf system of the source synchrotron, see Fig. ~\ref{bucket_label_phase_small}.  
...
\begin{figure}[!htb]
   \centering   
   \includegraphics*[width=130mm]{bucket_label_phase_small.jpg}
...
\begin{figure}[!htb]
   \centering   
   \includegraphics*[width=130mm]{bucket_label_frequency_small.jpg}
...
%The bucket label signal indicates neither all of the time ($f_{\mathit{rev}}^{l}$ or $f_{\mathit{rev}}^{s}$), when the first bucket of the target synchrotron passes by the rf virtual cavity, nor all of the time of the target synchrotron ($\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ or $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$), when two rf systems have a periodical phase difference. It indicates only the passing time of the first bucket, when it is correct aligned with the rf frequency ($\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ or $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$) of the source synchrotron.
...
Two synchrotrons are beating automatically. Two slightly different frequencies are $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$. When the large synchrotron is the target, the bucket label signal is with the frequency of $f_{\mathit{rev}}^{l}$, because $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}>=f_{\mathit{rev}}^{l}$. Tab.~\ref{Syn_rev_large_close} shows the formulas for the frequency of the bucket label signal, two slightly different frequencies for beating, the length of the synchronization window and the bunch and bucket center mismatch when the large synchrotron is the target.
...
When the small synchrotron is the target, the frequency of the bucket label signal is dependent on the relation between $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$ and $f_{\mathit{rev}}^{s}$. When $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}>=f_{\mathit{rev}}^{s}$, the revolution period is longer than the period for the phase alignment. Hence, the frequency of the bucket label signal is $f_{\mathit{rev}}^{s}$. Or the bucket label signal with the frequency of $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$. Tab.~\ref{Cir ratio close to integer} shows the formulas when the small synchrotron is the target.
...
Tab. ~\ref{Cir ratio far from integer large} shows the formulas for the frequency of the bucket label signal, two slightly different frequencies for beating, the length of the synchronization window and the bunch and bucket center mismatch when the large synchrotron is the target. Tab. ~\ref{Cir ratio far from integer small} shows them when the small synchrotron is the target.

\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization when circumference ratio is far away from an integer and the large synchrotron is the target}
\label{Cir ratio far from integer large}
\begin{center}
    \begin{tabular}{ | c | c | c |}
    \hline
	&  \multicolumn{2}{c|}{Large synchrotron is target synchrotron} \\ \hline
	Cases& \tabincell{c}{ $\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}>=f_{\mathit{rev}}^{l}$ \\ \\($\frac{Y}{n}>=1$) } &\tabincell{c}{ $\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}<f_{\mathit{rev}}^{l}$ \\ \\ ($\frac{Y}{n}<1$) } \\ \hline
   \tabincell{c}{Frequency \\ of bucket label} & $f_{\mathit{rev}}^{l}$ &$\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}$ \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} & \multicolumn{2}{c|}{$\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$ and $\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}$}\\ \hline
	\tabincell{c}{Beating\\ frequencies} & \multicolumn{2}{c|}{$\Delta f=|\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}-\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}|$}\\ \hline

	\tabincell{c}{Length of \\synchronization\\ window}& $2\times T_{\mathit{rev}}^{l}$ & $2\times [(h^l\times n)/Y]\times T_{\mathit{rf}}^{l}$\\ \hline
	\tabincell{c}{Bunch-to-bucket\\injection\\center\\ mismatch}&$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{l}}{1/\Delta f}\times360^\circ$ & $\pm\frac{1}{2}\times\frac{2\times [(h^l\times n)/Y]\times T_{\mathit{rf}}^{l}}{1/\Delta f}\times360^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Synchronization when circumference ratio is far away from an integer and the small synchrotron is the target}
\label{Cir ratio far from integer small}
\begin{center}
    \begin{tabular}{ | c | c | c |}
    \hline
	&  \multicolumn{2}{c|}{Small synchrotron is target synchrotron} \\ \hline
	Cases& \tabincell{c}{ $\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}>=f_{\mathit{rev}}^{s}$ \\ \\($\frac{Y}{m}>=1$) } &\tabincell{c}{ $\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}<f_{\mathit{rev}}^{s}$ \\ \\ ($\frac{Y}{m}<1$) } \\ \hline
   \tabincell{c}{Frequency \\ of bucket label} & $f_{\mathit{rev}}^{s}$ &$\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$ \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} & \multicolumn{2}{c|}{$\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$ and $\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}$}\\ \hline
	\tabincell{c}{Beating\\ frequencies} & \multicolumn{2}{c|}{$\Delta f=|\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}-\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}|$}\\ \hline

	\tabincell{c}{Length of \\synchronization\\ window}& $2\times T_{\mathit{rev}}^{s}$ & $2\times [(h^s\times m)/Y]\times T_{\mathit{rf}}^{s}$\\ \hline
	\tabincell{c}{Bunch-to-bucket\\injection\\center\\ mismatch}&$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{s}}{1/\Delta f}\times360^\circ$ & $\pm\frac{1}{2}\times\frac{2\times [(h^s\times m)/Y]\times T_{\mathit{rf}}^{s}}{1/\Delta f}\times360^\circ$\\ \hline
    \end{tabular}
\end{center}
\end{table}
There are various combination of $\frac{m}{n}$ and $\lambda$, $\lambda$ determines the beating speed. The smaller, the more precise bunch-to-bucket injection. $(h^l\times n)/Y$ and $(h^s\times m)/Y$ determines the two slightly different frequencies. The bigger $(h^l\times n)/Y$ and $(h^s\times m)/Y$, the smaller two slightly different frequencies, which has higher requirement for LLRF system. So we have to find a balance between the precision of the bunch-to-bucket injection and the low frequencies for beating.

Two slightly different frequencies could also be a fraction (between 1/Y and 1) times of the $\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$, see Sec. ~\ref{sec:close_to_int}.
...
\begin{center}
   ...
   \tabincell{c}{Frequency \\ of bucket label} & $\frac{f_{\mathit{rf}}^{\mathit{CR}}}{49}$  \\ \hline
...
\begin{center}
    ...
    \hline
	&  Small synchrotron (CR) is target synchrotron \\ \hline
   \tabincell{c}{Frequency \\ of bucket label} & $\frac{f_{\mathit{rf}}^{\mathit{CR}}}{11}$  \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} & $\frac{f_{\mathit{rf}}^{\mathit{SIS100}}}{5}=\SI{102.326}{\kHz}$ and $\frac{f_{\mathit{rf}}^{\mathit{CR}}}{11}=\SI{102.218}{\kHz}$\\ \hline
...
One bunch of CR is injected into one bucket of HESR. The beam is accumulated in HESR ~\cite{toelle_hesr_2007}. The large synchrotron is HESR and the small one is CR. $h^{\mathit{HESR}}=1$ and $h^{\mathit{CR}}=1$. The GCD of $h^{\mathit{HESR}}\times n=1\times5=5$ and $h^{\mathit{CR}} \times m=1\times 13=13$ is 1, namely $Y=1$. Substituting the circumference of HESR and CR to eq.~\ref{circumference_ratio_noint11}, we have
...
\begin{center}
    ...
    \hline
		&  Larger synchrotron (HESR) is target synchrotron \\ \hline
   		\tabincell{c}{Frequency \\ of bucket label} & $\frac{f_{\mathit{rf}}^{\mathit{HESR}}}{5}$  \\ \hline
		\rowcolor[gray]{0.8}
...
After the synchronization, the phase difference between the 1/13 CR and 1/5 HESR rf frequency markers depends on the accumulation method. Detailed parameter about the B2B transfer from CR to HESR, please see Appendix \ref{sec:CRtoHESR}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}
    ...
    \hline
	&  Small synchrotron (ESR) is target synchrotron \\ \hline
   \tabincell{c}{Frequency \\ of bucket label} & $\frac{f_{\mathit{rf}}^{\mathit{ESR}}}{9}$  \\ \hline
	\tabincell{c}{Two slightly \\different\\ frequencies} & $\frac{f_{\mathit{rf}}^{\mathit{SIS18}}}{5}=\SI{215.393}{\kHz}$ and $\frac{f_{\mathit{rf}}^{\mathit{ESR}}}{9}=\SI{219.642}{\kHz}$\\ \hline
...
In this section, all the synchronization methods are summarized.  Tab.~\ref{B2B_transfer_rule} summarizes the formulas when the revolution period is shorter than the period for the phase alignment between two rf systems. Tab.~\ref{B2B_transfer_rule1} summarizes the formulas when the revolution period is longer than the period for the phase alignment.
\begin{landscape} 
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Summary of the synchronization when the revolution period is shorter than the period for the phase alignment between two rf systems}{The formulas with black text are based on the assumption that the large synchrotron is the target and the formulas with red text the assumption that the small one is the target.}
...
\begin{table}[!htb]
\newcommand{\tabincell}[2]{\begin{tabular}{@{}#1@{}}#2\end{tabular}}
\caption{Summary of the synchronization when the revolution period is longer than the period for the phase alignment between two rf systems}{The formulas with black text are based on the assumption that the large synchrotron is the target and the formulas with red text the assumption that the small one is the target.}
\label{B2B_transfer_rule1}
\begin{center}
    \begin{tabular}{| c | c | c | c | c | c | c | c|}
    \hline
	\tabincell{c}{Circumference\\ratio} &  \tabincell{c}{rf cavity\\frequency ratio \\$f_{\mathit{rf}}^{l}/f_{\mathit{rf}}^{s}$}& \tabincell{c}{\tabincell{c}{Frequency \\ of bucket label}}&\tabincell{c}{Frequency beating\\Two slightly \\different frequencies}& \tabincell{c}{Frequency beating\\bunch-to-bucket center mismatch} \\ \hline
 \tabincell{c}{$C^l/C^s=\kappa$ \\Integer} &  \tabincell{c}{$\frac{h^l}{h^s\times \kappa}$\\ \\ $Y=GCD(h^l,h^s\times \kappa)$} & \tabincell{c}{$f_{\mathit{rev}}^{l}$  \\  \\ \color{red}{$f_{\mathit{rev}}^{s}$}} & \tabincell{c}{$\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}+\Delta f$\\ or\\ $\frac{f_{\mathit{rf}}^{l}}{h^l/Y}+\Delta f$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$}&\tabincell{c}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{l}}{1/\Delta f}\times360^\circ$ \\ \\ \color{red}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{s}}{1/\Delta f}\times360^\circ$}}\\ \hline
											
 	\tabincell{c}{$C^l/C^s=\kappa+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}=\kappa+ \lambda$\\close to integer }&\tabincell{c}{$\frac{h^l}{h^s\times (\kappa+ \lambda)}$\\ \\ $Y=GCD(h^l,h^s\times \kappa)$} & \tabincell{c}{$f_{\mathit{rev}}^{l}$  \\ \\ \color{red}{$f_{\mathit{rev}}^{s}$}} & \tabincell{c}{$\frac{f_{\mathit{rf}}^{l}}{h^l/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$ \\ $\Delta f=\frac{f_{\mathit{rf}}^{l}}{h^l/Y}-\frac{f_{\mathit{rf}}^{s}}{(h^s\times \kappa)/Y}$} & \tabincell{c}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{l}}{1/\Delta f}\times360^\circ$ \\  \\ \color{red}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{s}}{1/\Delta f}\times360^\circ$}}  \\ \hline


\tabincell{c}{$C^l/C^s=m/n+ \lambda$ \\ or \\ $f{\mathit{rev}}^{s}/f{\mathit{rev}}^{l}=m/n+ \lambda$\\far away from integer}&\tabincell{c}{ $\frac{h^l}{h^s \times (m/n+ \lambda)}$\\ \\ \tabincell{c}{Y=GCD\\$(h^l\times n,h^s \times m)$}}&\tabincell{c}{$f_{\mathit{rev}}^{l}$  \\  \\ \color{red}{$f_{\mathit{rev}}^{s}$}} & \tabincell{c}{$\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}$ and $\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$ \\$ \Delta f=\frac{f_{\mathit{rf}}^{l}}{(h^l\times n)/Y}-\frac{f_{\mathit{rf}}^{s}}{(h^s\times m)/Y}$} & \tabincell{c}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{l}}{1/\Delta f}\times360^\circ$ \\  \\ \color{red}{$\pm\frac{1}{2}\times\frac{2\times T_{\mathit{rev}}^{s}}{1/\Delta f}\times360^\circ$}}  \\ \hline
